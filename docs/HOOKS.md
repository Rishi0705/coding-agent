...
Hooks let you intercept and control every tool call the agent makes — before it runs, after it runs, or both. They are compatible with the Claude Code `.claude/settings.json` format, so any hook script that works in Claude Code works in Coco Codes out of the box.
...
Restart Coco Codes — any attempt to run a `git` command will be blocked cleanly.
...
**Coco Codes tool name mapping:**

| Claude Code Name | Coco Codes Internal Name |
|-----------------|--------------------------|
...
2. `~/.coco_codes/hooks.json` — global user hooks

---

## Implementation Notes

The hook engine lives in `coco_codes/hook_engine/` and is a self-contained library with no dependency on the rest of Coco Codes.

Hooks are injected at the `pydantic-ai` `ToolManager._call_tool()` level via a patch in `coco_codes/pydantic_patches.py`, so they fire on every tool call regardless of which agent or model is in use.
