...
Agent Skills are reusable, modular capabilities that extend Coco Codes's functionality. Think of them as specialized training packets you can dynamically load when needed—that extend the agent dynamically.
...
Skills are installed by placing them in designated skill directories. Coco Codes scans these directories at startup to discover available skills.

### Default Skill Directories

By default, Coco Codes looks for skills in:

1. **`~/.coco_codes/skills/`** - User-level skills (global)
...
2. **Download or clone a skill** into the directory:
   ...
   cd ~/.coco_codes/skills
   git clone https://github.com/example/coco-codes-docker.git docker
...
3. **Verify the skill** has a `SKILL.md` file:
   ...
   ```bash
   ls ~/.coco_codes/skills/my-custom-skill/SKILL.md
   ```
...
Coco Codes provides an interactive TUI (Text User Interface) for managing skills.
...
When skills are enabled, Coco Codes automatically injects available skills into the system prompt:
...
1. **Discovery** → Coco Codes scans skill directories at startup
...
1. Place your skill in `~/.coco_codes/skills/`
...
Agent Skills can be configured through Coco Codes's configuration system.
...
| `skill_directories` | JSON list | `["~/.coco_codes/skills", "./skills"]` | Directories to scan for skills |
...
/set skill_directories = "[\"/path/to/skills\", \"~/.coco_codes/skills\"]"
...
Settings are stored in `~/.coco_codes/coco.cfg`:

```ini
[coco]
skills_enabled = true
skill_directories = ["/home/user/.coco_codes/skills", "./skills"]
...
⚠️ **Important:** Skills execute with the same permissions as Coco Codes. Follow these security best practices:
...
- Any files Coco Codes has access to

**Recommendation:** Run Coco Codes with minimal necessary permissions.
...
3. For core skills functionality issues, report to Coco Codes
