"""
Models development API parser for Coco Codes.

This module provides functionality to parse and work with the models.dev API,
including provider and model information, search capabilities, and conversion to Coco Codes
...
# Provider type mapping for Coco Codes configuration
PROVIDER_TYPE_MAP = {
    "anthropic": "anthropic",
    "openai": "openai",
    "google": "gemini",
...
def convert_to_coco_codes_config(
    model: ModelInfo, provider: ProviderInfo
) -> Dict[str, Any]:
    """
    Convert a model and provider to Coco Codes configuration format.
    ...
    Returns:
        Dictionary in Coco Codes configuration format
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Example: Convert to Coco Codes config
        if providers and registry.get_models():
            provider = providers[0]
            models = registry.get_models(provider.id)
            if models:
                config = convert_to_coco_codes_config(models[0], provider)
