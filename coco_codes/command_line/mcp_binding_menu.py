...
Launched from :mod:`coco_codes.command_line.agent_menu` (and reused by the
post-install flow in :mod:`coco_codes.command_line.mcp.install_command`).
...
from coco_codes.mcp_.agent_bindings import (
    get_bound_servers,
    is_bound,
    set_binding,
    toggle_auto_start,
...
from coco_codes.messaging import emit_info, emit_warning
...
async def prompt_bind_after_install(server_name: str) -> None:
    ...
    """
    from coco_codes.agents import get_available_agents
    from coco_codes.mcp_.agent_bindings import (
        get_auto_start,
        toggle_binding,
    )
