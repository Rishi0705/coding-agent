"""Command handlers for Coco Codes - CONFIG commands.
...
from coco_codes.command_line.command_registry import register_command
from coco_codes.config import get_config_keys
...
def get_commands_help():
    ...
    from coco_codes.command_line.command_handler import get_commands_help as _gch
...
def handle_reasoning_command(command: str) -> bool:
    ...
    from coco_codes.messaging import emit_error, emit_success, emit_warning
    ...
    effort = tokens[1]
    try:
        from coco_codes.config import set_openai_reasoning_effort
    ...
    from coco_codes.config import get_openai_reasoning_effort
...
def handle_verbosity_command(command: str) -> bool:
    ...
    verbosity = tokens[1]
    try:
        from coco_codes.config import set_openai_verbosity
    ...
    from coco_codes.config import get_openai_verbosity
...
def handle_set_command(command: str) -> bool:
    ...
    from coco_codes.messaging import emit_error, emit_info, emit_success, emit_warning
    ...
    if key:
        ...
        if key == "cancel_agent_key":
            from coco_codes.keymap import VALID_CANCEL_KEYS
...
def _get_json_agents_pinned_to_model(model_name: str) -> list:
    ...
    from coco_codes.agents.json_agent import discover_json_agents
...
def handle_pin_model_command(command: str) -> bool:
    ...
    if len(tokens) != 3:
        ...
        # Get built-in agents
        from coco_codes.agents.agent_manager import get_agent_descriptions
    ...
    try:
        if is_json_agent:
            ...
            from coco_codes.config import set_agent_pinned_model
...
def handle_unpin_command(command: str) -> bool:
    ...
    from coco_codes.agents.json_agent import discover_json_agents
    from coco_codes.config import get_agent_pinned_model
    from coco_codes.messaging import emit_error, emit_info, emit_success, emit_warning
    ...
    try:
        if is_json_agent:
            ...
            from coco_codes.config import clear_agent_pinned_model
...
def handle_diff_command(command: str) -> bool:
    ...
    from coco_codes.command_line.diff_menu import interactive_diff_picker
    from coco_codes.config import (
...
def handle_colors_command(command: str) -> bool:
    ...
    from coco_codes.command_line.colors_menu import interactive_colors_picker
    from coco_codes.config import set_banner_color
