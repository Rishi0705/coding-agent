...
from coco_codes.command_line.utils import safe_input
from coco_codes.config import EXTRA_MODELS_FILE, set_config_value
from coco_codes.list_filtering import query_matches_text
from coco_codes.messaging import emit_error, emit_info, emit_warning
from coco_codes.models_dev_parser import ModelInfo, ModelsDevRegistry, ProviderInfo
...
class AddModelMenu:
    ...
    def _add_model_to_extra_config(
        ...
        try:
            ...
            # Convert to Coco Codes config format (dictionary value)
    ...
    def _build_model_config(self, model: ModelInfo, provider: ProviderInfo) -> dict:
        """Build a Coco Codes compatible model configuration.
        ...
        """
        # Map provider IDs to Coco Codes types
        type_mapping = {
            "openai": "openai",
            "anthropic": "anthropic",
            "google": "gemini",
    ...
    def run(self) -> bool:
        ...
        try:
            ...
            # This is needed because coco_codes runs in an async context
