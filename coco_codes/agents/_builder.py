...
hook; see :func:`coco_codes.callbacks.on_wrap_pydantic_agent`.
"""
...
from coco_codes.agents._compaction import make_history_processor
from coco_codes.agents._steer_processor import make_steer_history_processor
from coco_codes.agents.event_stream_handler import event_stream_handler
...
from coco_codes.mcp_ import get_mcp_manager
from coco_codes.messaging import emit_error, emit_info, emit_warning
...
def load_agent_rules() -> Optional[str]:
    ...
    Global rules (``~/.coco_codes/AGENTS.md``) come first; project-local rules
    are appended, allowing projects to override/extend global ones.

    **Search order for project rules:**

    1. ``.coco_codes/AGENTS.md`` (preferred — keeps root clean)
...
def _iter_autostart_targets(manager: Any, agent_name: str):
    ...
    """
    try:
        from coco_codes.mcp_.agent_bindings import get_bound_servers
...
def _assemble_instructions(agent: Any, resolved_model_name: str) -> str:
    ...
    from coco_codes.model_utils import prepare_prompt_for_model
