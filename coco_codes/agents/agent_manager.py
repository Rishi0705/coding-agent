...
from coco_codes.callbacks import on_agent_reload, on_register_agents
...
def _discover_agents(message_group_id: Optional[str] = None):
    ...
    import coco_codes.agents as agents_package
    ...
    for _, modname, _ in pkgutil.iter_modules(agents_package.__path__):
        ...
        try:
            # Import the module
            module = importlib.import_module(f"coco_codes.agents.{modname}")
    ...
    for _, subpkg_name, ispkg in pkgutil.iter_modules(agents_package.__path__):
        ...
        try:
            ...
            subpkg = importlib.import_module(f"coco_codes.agents.{subpkg_name}")
            ...
            for _, modname, _ in pkgutil.iter_modules(subpkg.__path__):
                ...
                try:
                    ...
                    module = importlib.import_module(
                        f"coco_codes.agents.{subpkg_name}.{modname}"
                    )
...
def get_current_agent_name() -> str:
    ...
    Returns:
        ...
        Priority: session agent > config default > 'coco-codes'.
    """
...
def load_agent(agent_name: str) -> BaseAgent:
    ...
    if agent_name not in _AGENT_REGISTRY:
        # Fallback to coco-codes if agent not found
        if "coco-codes" in _AGENT_REGISTRY:
            agent_name = "coco-codes"
        else:
            raise ValueError(
