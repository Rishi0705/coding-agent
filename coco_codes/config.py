...
from coco_codes.session_storage import save_session


def _get_xdg_dir(env_var: str, fallback: str) -> str:
    """
    Get directory for coco_codes files, defaulting to ~/.coco_codes.

    XDG paths are only used when the corresponding environment variable
    is explicitly set by the user. Otherwise, we use the legacy ~/.coco_codes
    ...
    Returns:
        Path to the directory for coco_codes files
    """
    # Use XDG directory ONLY if environment variable is explicitly set
    xdg_base = os.getenv(env_var)
    if xdg_base:
        return os.path.join(xdg_base, "coco_codes")

    # Default to legacy ~/.coco_codes for all file types
    return os.path.join(os.path.expanduser("~"), ".coco_codes")
...
CONFIG_FILE = os.path.join(CONFIG_DIR, "coco.cfg")
...
def ensure_config_exists():
    """
    Ensure that XDG directories and coco.cfg exist, prompting if needed.
    ...
    if missing:
        ...
        for key in missing:
            if key == "assistant_name":
                ...
                val = input(
                    "What's your name (so Coco Codes knows its owner)? "
                ).strip()
            else:
...
def get_config_keys():
    """
    Returns the list of all config keys currently in coco.cfg,
...
def load_mcp_server_configs():
    """
    Loads the MCP server configurations from XDG_CONFIG_HOME/coco_codes/mcp_servers.json.
    Returns a dict mapping names to their URL or config dict.
    If file does not exist, returns an empty dict.
    """
    from coco_codes.messaging.message_queue import emit_error
...
def get_global_model_name():
    """Return a valid model name for Coco Codes to use.
    ...
    2. Otherwise, look at ``model`` in *coco.cfg*
...
def get_user_agents_directory() -> str:
    ...
    Returns:
        Path to the user's Coco Codes agents directory.
    """
...
def get_project_agents_directory() -> Optional[str]:
    ...
    Looks for a .coco_codes/agents/ directory in the current working directory.
    ...
    """
    project_agents_dir = os.path.join(os.getcwd(), ".coco_codes", "agents")
    ...
    if not command_history_exists:
        try:
            ...
            old_history_file = os.path.join(
                os.path.expanduser("~"), ".coco_codes_history.txt"
            )
...
def get_yolo_mode():
    """
    Checks coco.cfg for 'yolo_mode' (case-insensitive in value only).
...
def get_safety_permission_level():
    """
    Checks coco.cfg for 'safety_permission_level' (case-insensitive in value only).
...
def get_mcp_disabled():
    """
    Checks coco.cfg for 'disable_mcp' (case-insensitive in value only).
    Defaults to False if not set.
    Allowed values for ON: 1, '1', 'true', 'yes', 'on' (all case-insensitive for value).
    When enabled, Coco Codes will skip loading MCP servers entirely.
    """
    true_vals = {"1", "true", "yes", "on"}
...
def get_grep_output_verbose():
    """
    Checks coco.cfg for 'grep_output_verbose' (case-insensitive in value only).
...
def get_auto_save_session() -> bool:
    """
    Checks coco.cfg for 'auto_save_session' (case-insensitive in value only).
    Defaults to True if not set.
    Allowed values for ON: 1, '1', 'true', 'yes', 'on' (all case-insensitive for value).
    """
    true_vals = {"1", "true", "yes", "on"}
...
def get_suppress_thinking_messages() -> bool:
    """
    Checks coco.cfg for 'suppress_thinking_messages' (case-insensitive in value only).
    Defaults to False if not set.
    Allowed values for ON: 1, '1', 'true', 'yes', 'on' (all case-insensitive for value).
...
def get_suppress_informational_messages() -> bool:
    """
    Checks coco.cfg for 'suppress_informational_messages' (case-insensitive in value only).
    Defaults to False if not set.
    Allowed values for ON: 1, '1', 'true', 'yes', 'on' (all case-insensitive for value).
...
def load_api_keys_to_environment():
    """Load all API keys from .env and coco.cfg into environment variables.

    Priority order:
    1. .env file (highest priority) - if present in current directory
    2. coco.cfg - fallback if not in .env
    ...
    # Step 2: Load from coco.cfg, but only if not already set
    # This ensures .env has priority over coco.cfg
...
def get_default_agent() -> str:
    """
    Get the default agent name from coco.cfg.

    Returns:
        str: The default agent name, or "coco-codes" if not set.
    """
    return get_value("default_agent") or "coco-codes"


def set_default_agent(agent_name: str) -> None:
    """
    Set the default agent name in coco.cfg.
