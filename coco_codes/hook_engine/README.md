...
```python
from coco_codes.hook_engine import HookEngine, EventData
...
Hooks can be written using **either** the provider's tool name **or** coco_codes's
internal tool name — the matcher treats them as equivalent.

### Claude Code → coco_codes

| Claude Code (`matcher`) | coco_codes internal | Notes |
|-------------------------|---------------------|-------|
