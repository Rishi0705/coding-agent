"""
Tool name alias registry — maps each AI provider's tool names to coco_codes's
...
Each provider block defines a dict[str, str] mapping:
    "<Provider tool name>" -> "<coco_codes internal tool name>"
...
CLAUDE_CODE_ALIASES: Dict[str, str] = {
    ...
    # NOTE: the tools below have no direct coco_codes equivalent yet.
...
GEMINI_ALIASES: Dict[str, str] = {
    # Add Gemini → coco_codes tool mappings here
}
...
CODEX_ALIASES: Dict[str, str] = {
    # Add Codex → coco_codes tool mappings here
}
...
SWARM_ALIASES: Dict[str, str] = {
    # Add Swarm → coco_codes tool mappings here
}
...
def resolve_internal_name(provider_tool_name: str) -> Optional[str]:
    """
    Return the coco_codes internal tool name for a given provider tool name,
