...
from coco_codes.plugins.agent_skills.discovery import refresh_skill_cache
from coco_codes.plugins.agent_skills.installer import InstallResult

logger = logging.getLogger(__name__)

_DEFAULT_SKILLS_DIR = Path.home() / ".coco_codes" / "skills"
...
def _download_to_file(url: str, dest: Path) -> bool:
    ...
    headers = {
        ...
        "User-Agent": "coco-codes/skill-downloader",
    }
...
def download_and_install_skill(
    ...
    Args:
        ...
        target_dir: Base skills directory. Defaults to ~/.coco_codes/skills.
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="coco_codes_skill_") as tmp:
            tmp_dir = Path(tmp)
