...
from coco_codes.plugins.chatgpt_oauth import config, utils
...
def test_config_paths():
    ...
    # XDG paths use "coco_codes" (without dot) in ~/.local/share or ~/.config
    assert "coco_codes" in str(token_path)

    config_dir = config.get_config_dir()
    # Default is ~/.coco_codes; XDG paths only used when XDG env vars are set
    assert config_dir.name in ("coco_codes", ".coco_codes")
...
@patch("coco_codes.plugins.chatgpt_oauth.utils.requests.post")
...
@patch("coco_codes.plugins.chatgpt_oauth.utils.requests.get")
