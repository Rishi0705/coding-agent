...
> For **built-in commands**, see the built-in command files in `coco_codes/command_line/`.

## Overview

This plugin demonstrates how to create custom commands using Coco Codes's callback system.

**Important**: Custom commands use `register_callback()`, NOT `@register_command`.

## Command Types in Coco Codes

### 1. Built-in Commands (Core Functionality)
- Use `@register_command` decorator
- Located in `coco_codes/command_line/core_commands.py`, `session_commands.py`, `config_commands.py`
...
```
coco_codes/plugins/example_custom_command/
...
```bash
mkdir -p coco_codes/plugins/my_plugin
touch coco_codes/plugins/my_plugin/__init__.py
touch coco_codes/plugins/my_plugin/register_callbacks.py
```

### Step 2: Implement Callbacks

```python
# coco_codes/plugins/my_plugin/register_callbacks.py

from coco_codes.callbacks import register_callback
from coco_codes.messaging import emit_info, emit_success
...
```bash
# Restart Coco Codes to load the plugin
coco-codes
...
- **Don't modify global state**: Use Coco Codes's config system
...
from coco_codes.plugins.my_plugin.register_callbacks import _handle_custom_command
...
- `coco_codes/callbacks.py` - Callback system implementation
- `coco_codes/command_line/command_handler.py` - Command dispatcher
- `coco_codes/command_line/core_commands.py` - Example built-in commands
- `coco_codes/command_line/command_registry.py` - Registry system

## Questions?

If you're unsure whether to create a custom command or a built-in command:

- **Is it core Coco Codes functionality?** → Use `@register_command` (built-in)
