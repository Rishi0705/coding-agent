# Azure AI Foundry Plugin for Coco Codes
...
4. **Python packages**: `azure-identity>=1.15.0` (installed with Coco Codes)
...
Models are stored in `~/.coco_codes/extra_models.json`:
...
4. **Integration**: Wraps the client in pydantic-ai's `AnthropicModel` for
   seamless integration with Coco Codes's agent system
...
```
coco_codes/plugins/azure_foundry/
...
This plugin follows Coco Codes's plugin architecture. Key callbacks:
...
Same as Coco Codes (see repository LICENSE file).
