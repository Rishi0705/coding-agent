...
def _patched_estimate_tokens(text: str) -> int:
    ...
    """
    from coco_codes.plugins.token_ratio_learner.ratios import count_tokens
...
def _patched_estimate_tokens_for_message(
    ...
    from coco_codes.agents._history import stringify_part
    from coco_codes.plugins.token_ratio_learner.ratios import count_tokens
    ...
    **kwargs: Any,
    ...
    try:
        ...
        if input_tokens > 0 and input_char_count > 0:
            from coco_codes.plugins.token_ratio_learner.ratios import (
                _record_token_ratio,
            )
...
def _on_startup() -> None:
    ...
    from coco_codes.agents import _history
    ...
    # NOTE: ``coco_codes.agents.subagent_stream_handler`` re-exports the
    # *function* ``subagent_stream_handler``, not the module.  We must
    # reach the actual module via ``importlib``.
    import importlib

    ssh_mod = importlib.import_module("coco_codes.agents.subagent_stream_handler")
