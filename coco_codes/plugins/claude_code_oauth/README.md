...
This plugin adds OAuth authentication for Claude Code to Coco Codes, automatically importing available models into your configuration.
...
- **Token Management**: Secure storage of OAuth tokens in the Coco Codes config directory
...
After authentication, models will be added to `~/.coco_codes/extra_models.json`:
...
- **Token Storage**: Tokens are saved to `~/.coco_codes/claude_code_oauth.json` with `0o600` permissions
...
- Ensure the browser completed the redirect back to Coco Codes (no pop-up blockers)
...
- **Plugin Registration**: Custom command handlers wired into Coco Codes
