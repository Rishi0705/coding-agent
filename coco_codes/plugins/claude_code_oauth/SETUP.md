...
This guide walks you through using the Claude Code OAuth plugin inside Coco Codes.

## Quick Start

1. Ensure the plugin files live under `coco_codes/plugins/claude_code_oauth/`
2. Restart Coco Codes so it loads the plugin
...
5. Tokens are stored locally at `~/.coco_codes/claude_code_oauth.json`
...
- Models appear in `~/.coco_codes/extra_models.json` with the `claude-code-` prefix
...
```bash
python coco_codes/plugins/claude_code_oauth/test_plugin.py
```
