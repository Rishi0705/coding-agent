...
USER_PLUGINS_DIR = Path.home() / ".coco_codes" / "plugins"
...
def _load_builtin_plugins(plugins_dir: Path) -> list[str]:
    ...
    from coco_codes.config import get_safety_permission_level
    ...
    for item in plugins_dir.iterdir():
        if item.is_dir() and not item.name.startswith("_"):
            ...
            if callbacks_file.exists():
                ...
                try:
                    module_name = f"coco_codes.plugins.{plugin_name}.register_callbacks"
...
def _load_user_plugins(user_plugins_dir: Path) -> list[str]:
    """Load user plugins from ~/.coco_codes/plugins/.
...
def load_plugin_callbacks() -> dict[str, list[str]]:
    ...
    Loads plugins from:
    1. Built-in plugins in the coco_codes/plugins/ directory
    2. User plugins in ~/.coco_codes/plugins/
