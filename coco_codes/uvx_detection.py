"""Detect if coco-codes was launched via uvx on Windows.

This module provides utilities to detect the launch method of coco-codes,
specifically to handle signal differences when running via uvx on Windows.

On Windows, when launched via `uvx coco-codes`, Ctrl+C (SIGINT) gets captured
...
def is_launched_via_uvx() -> bool:
    """Detect if coco-codes was launched via uvx.
