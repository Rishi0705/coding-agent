...
from coco_codes.session_storage import SessionMetadata


@pytest.fixture
def mock_config_paths(monkeypatch):
    mock_home = "/mock_home"
    mock_config_dir = os.path.join(mock_home, ".coco_codes")
...
class TestAutoSaveSessionFunctionality:
    @patch("coco_codes.config.get_auto_save_session")
    ...
    @patch("coco_codes.config.save_session")
    @patch("coco_codes.config.get_current_autosave_session_name")
    @patch("coco_codes.config.datetime.datetime")
    @patch("coco_codes.config.get_auto_save_session")
...
class TestFinalizeAutoSaveSession:
    @patch("coco_codes.config.rotate_autosave_id", return_value="fresh_id")
    @patch("coco_codes.config.auto_save_session_if_enabled", return_value=True)
    ...
    @patch("coco_codes.config.rotate_autosave_id", return_value="fresh_id")
    @patch("coco_codes.config.auto_save_session_if_enabled", return_value=False)
