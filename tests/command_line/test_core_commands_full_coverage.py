"""Full coverage tests for coco_codes/command_line/core_commands.py."""
...
class TestGetCommandsHelp:
    def test_lazy_import(self):
        from coco_codes.command_line.core_commands import get_commands_help
...
class TestHandleHelpCommand:
    def test_help(self):
        from coco_codes.command_line.core_commands import handle_help_command
...
class TestHandleToolsCommand:
    def test_tools(self):
        from coco_codes.command_line.core_commands import handle_tools_command
...
class TestHandlePasteCommand:
    def test_no_image(self):
        from coco_codes.command_line.core_commands import handle_paste_command

        with (
            patch(
                "coco_codes.command_line.clipboard.has_image_in_clipboard",
                return_value=False,
            ),
    ...
    def test_success(self):
        ...
        with (
            ...
            patch(
                "coco_codes.command_line.clipboard.capture_clipboard_image_to_pending",
                return_value="img.png",
            ),
            patch(
                "coco_codes.command_line.clipboard.get_clipboard_manager",
...
class TestHandleTutorialCommand:
    def test_chatgpt(self):
        from coco_codes.command_line.core_commands import handle_tutorial_command

        with (
            patch("coco_codes.command_line.onboarding_wizard.reset_onboarding"),
            patch("concurrent.futures.ThreadPoolExecutor") as pool,
            patch("coco_codes.messaging.emit_info"),
            patch("coco_codes.plugins.chatgpt_oauth.oauth_flow.run_oauth_flow"),
            patch("coco_codes.model_switching.set_model_and_reload_agent"),
        ):
...
class TestHandleExitCommand:
    def test_exit(self):
        from coco_codes.command_line.core_commands import handle_exit_command
    ...
    def test_exit_error(self):
        ...
        with patch("coco_codes.messaging.emit_success", side_effect=Exception):
            assert handle_exit_command("/exit") is True


class TestHandleAgentCommand:
    def _mock_agent(self, name="coco-codes", display="Coco Codes", desc="A dog"):
        a = MagicMock()
        a.name = name
    ...
    def test_no_args_interactive_select(self):
        from coco_codes.command_line.core_commands import handle_agent_command
        ...
        with (
            ...
            patch(
                "coco_codes.command_line.core_commands.finalize_autosave_session",
                return_value="sess1",
            ),
            patch("coco_codes.agents.set_current_agent", return_value=True),
            patch("coco_codes.messaging.emit_success"),
            patch("coco_codes.messaging.emit_info"),
        ):
            mock_future = MagicMock()
    ...
    def test_no_args_already_current(self):
        ...
        with (
            patch("concurrent.futures.ThreadPoolExecutor") as pool,
            patch("coco_codes.agents.get_current_agent", return_value=agent),
            patch("coco_codes.messaging.emit_info"),
        ):
            mock_future = MagicMock()
            mock_future.result.return_value = "coco-codes"
            pool.return_value.__enter__ = MagicMock(return_value=pool.return_value)
            pool.return_value.__exit__ = MagicMock(return_value=False)
            pool.return_value.submit.return_value = mock_future
            assert handle_agent_command("/agent") is True
    ...
    def test_no_args_switch_fails(self):
        ...
        with (
            ...
            patch("coco_codes.agents.set_current_agent", return_value=False),
            patch("coco_codes.messaging.emit_warning"),
        ):
            mock_future = MagicMock()
    ...
    def test_no_args_picker_fails_fallback(self):
        ...
        with (
            ...
            patch(
                ...
                return_value={"coco-codes": "Coco Codes"},
            ),
            patch(
    ...
    def test_with_name_switch(self):
        ...
        with (
            ...
            patch(
                "coco_codes.agents.get_available_agents",
                return_value={"coco-codes": "CP", "other": "Other"},
            ),
            patch(
                "coco_codes.command_line.core_commands.finalize_autosave_session",
                return_value="s",
    ...
    def test_with_name_already_current(self):
        ...
        with (
            ...
            assert handle_agent_command("/agent coco-codes") is True
    ...
    def test_with_name_switch_fails(self):
        ...
        with (
            ...
            patch(
                "coco_codes.agents.get_available_agents",
                return_value={"coco-codes": "CP", "other": "O"},
            ),
            patch(
                "coco_codes.command_line.core_commands.finalize_autosave_session",
                return_value="s",
...
class TestHandleModelCommand:
    def test_no_args_interactive_select(self):
        from coco_codes.command_line.core_commands import handle_model_command

        with (
            patch("concurrent.futures.ThreadPoolExecutor") as pool,
            patch("coco_codes.command_line.model_picker_completion.set_active_model"),
            patch("coco_codes.messaging.emit_success"),
        ):
            mock_future = MagicMock()
    ...
    def test_with_model_name_matched(self):
        ...
        with (
            patch(
                "coco_codes.command_line.core_commands.update_model_in_input",
                return_value="done",
            ),
            patch(
...
class TestHandleAddModelCommand:
    def test_success(self):
        from coco_codes.command_line.core_commands import handle_add_model_command

        with (
            patch("coco_codes.tools.command_runner.set_awaiting_user_input"),
            patch(
                "coco_codes.command_line.add_model_menu.interactive_model_picker",
                return_value=True,
            ),
            patch("coco_codes.messaging.emit_info"),
        ):
...
class TestHandleModelSettingsCommand:
    def test_show_flag(self):
        from coco_codes.command_line.core_commands import handle_model_settings_command

        with patch(
            "coco_codes.command_line.model_settings_menu.show_model_settings_summary"
        ):
    ...
    def test_interactive_success(self):
        ...
        with (
            patch("coco_codes.tools.command_runner.set_awaiting_user_input"),
            patch(
                "coco_codes.command_line.model_settings_menu.interactive_model_settings",
                return_value=True,
            ),
            patch("coco_codes.messaging.emit_success"),
            patch("coco_codes.messaging.emit_info"),
...
class TestHandleWiggumCommand:
    def test_no_prompt(self):
        from coco_codes.plugins.wiggum.register_callbacks import (
            handle_wiggum_command,
        )

        with (
            patch("coco_codes.plugins.wiggum.register_callbacks.emit_warning"),
            patch("coco_codes.plugins.wiggum.register_callbacks.emit_info"),
        ):
    ...
    def test_with_prompt(self):
        ...
        with (
            patch("coco_codes.plugins.wiggum.state.start"),
            patch("coco_codes.plugins.wiggum.register_callbacks.emit_success"),
            patch("coco_codes.plugins.wiggum.register_callbacks.emit_info"),
        ):
...
class TestHandleWiggumStopCommand:
    def test_active(self):
        ...
        with (
            patch(
                "coco_codes.plugins.wiggum.state.is_active",
                return_value=True,
            ),
            patch("coco_codes.plugins.wiggum.state.stop"),
            patch("coco_codes.plugins.wiggum.register_callbacks.emit_success"),
        ):
...
class TestHandleMcpCommand:
    def test_delegates(self):
        from coco_codes.command_line.core_commands import handle_mcp_command

        mock_handler = MagicMock()
        mock_handler.handle_mcp_command.return_value = True
        with patch(
            "coco_codes.command_line.mcp.MCPCommandHandler", return_value=mock_handler
        ):
