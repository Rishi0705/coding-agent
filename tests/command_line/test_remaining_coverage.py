...
def test_clipboard_pil_import_failure():
    ...
    mod_name = "coco_codes.command_line.clipboard"
...
def test_load_context_completion_exception():
    ...
    with patch("coco_codes.command_line.load_context_completion.Path") as mock_path:
...
def test_file_path_completion_permission_error():
    ...
    with patch(
        "coco_codes.command_line.file_path_completion.glob.glob",
        side_effect=PermissionError("denied"),
    ):
...
def test_prompt_toolkit_main_block():
    ...
    import coco_codes.command_line.prompt_toolkit_completion as mod
...
def test_command_handler_markdown_import_failure():
    ...
    from coco_codes.command_line.command_handler import handle_command
    ...
    with patch(
        "coco_codes.callbacks.on_custom_command", return_value=["some result"]
    ) as _mock_cb:
        with patch.dict(
            sys.modules,
            {
                "coco_codes.plugins.customizable_commands": None,
                "coco_codes.plugins.customizable_commands.register_callbacks": None,
            },
        ):
...
def test_pin_completion_empty_partial_model():
    ...
    from coco_codes.command_line.pin_command_completion import PinCompleter
    ...
    with (
        patch(
            "coco_codes.command_line.pin_command_completion.load_model_names",
...
async def test_diff_menu_keybindings():
    ...
    from coco_codes.command_line.diff_menu import _split_panel_selector
...
def test_config_set_compaction_strategy_not_in_keys():
    ...
    with patch(
        "coco_codes.command_line.config_commands.get_config_keys", return_value=[]
    ):
...
def test_config_set_agent_reload_failure():
    ...
    with (
        ...
        patch("coco_codes.messaging.emit_warning") as _mock_warn,
...
def test_config_pin_json_agent_reload_failure():
    ...
    with (
        ...
        patch(
            "coco_codes.agents.agent_manager.get_agent_descriptions", return_value={}
        ),
        patch("coco_codes.messaging.emit_info"),
        patch("coco_codes.messaging.emit_success"),
        patch("coco_codes.messaging.emit_warning") as _mock_warn,
...
def test_config_pin_list_json_agents_with_pinned():
    ...
    with (
        ...
        patch("coco_codes.messaging.emit_info") as _mock_info,
        patch("coco_codes.messaging.emit_warning"),
    ):
        result = handle_pin_model_command("/pin_model")
        assert result is True
...
async def test_colors_menu_split_panel_selector():
    ...
    with patch("coco_codes.command_line.colors_menu.Application") as mock_app_cls:
        mock_app = AsyncMock()
        mock_app_cls.return_value = mock_app
        mock_app.run_async = AsyncMock()
...
def test_model_settings_start_editing_choice():
    ...
    with patch(
        "coco_codes.command_line.model_settings_menu._get_setting_choices",
...
def test_autosave_render_message_browser():
    ...
    from coco_codes.command_line.autosave_menu import _render_message_browser_panel
...
def test_autosave_line_144():
    ...
    from coco_codes.command_line.autosave_menu import _get_session_entries
...
def test_autosave_line_317():
    ...
    from coco_codes.command_line.autosave_menu import _render_preview_panel
...
def test_agent_menu_render_preview_no_entry():
    ...
    from coco_codes.command_line.agent_menu import _render_preview_panel
...
def test_uc_menu_toggle_tool_meta_not_found():
    ...
    from coco_codes.command_line.uc_menu import _toggle_tool_enabled
    ...
    with tempfile.NamedTemporaryFile(mode="w", suffix=".py", delete=False) as f:
        ...
        with patch("coco_codes.command_line.uc_menu.emit_error"):
...
def test_uc_menu_delete_tool():
    ...
    from coco_codes.command_line.uc_menu import _delete_tool
    from coco_codes.plugins.universal_constructor.models import ToolMeta, UCToolInfo
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with (
            patch("coco_codes.plugins.universal_constructor.USER_UC_DIR", Path(tmpdir)),
            patch("coco_codes.command_line.uc_menu.emit_success"),
        ):
...
def test_uc_menu_delete_tool_exception():
    ...
    with (
        patch("coco_codes.command_line.uc_menu.Path") as mock_path,
        patch("coco_codes.command_line.uc_menu.emit_error"),
    ):
...
def test_uc_menu_highlight_python_line():
    ...
    from coco_codes.command_line.uc_menu import _highlight_python_line
...
def test_uc_menu_render_source_panel():
    ...
    from coco_codes.command_line.uc_menu import _render_source_panel
    from coco_codes.plugins.universal_constructor.models import ToolMeta, UCToolInfo
...
def test_uc_menu_render_preview_panel_with_author():
    ...
    from coco_codes.command_line.uc_menu import _render_preview_panel
    from coco_codes.plugins.universal_constructor.models import ToolMeta, UCToolInfo
...
def test_uc_menu_render_menu_panel_variants():
    ...
    from coco_codes.command_line.uc_menu import _render_menu_panel
    from coco_codes.plugins.universal_constructor.models import ToolMeta, UCToolInfo
...
def test_uc_load_source_code():
    ...
    from coco_codes.command_line.uc_menu import _load_source_code
    from coco_codes.plugins.universal_constructor.models import ToolMeta, UCToolInfo
...
def test_core_commands_shlex_fallback():
    ...
    from coco_codes.command_line.core_commands import handle_cd_command

    # Unbalanced quotes will cause shlex.split to fail, triggering fallback
    with patch("coco_codes.command_line.core_commands.emit_error"):
...
def test_onboarding_slides_import_error():
    ...
    with patch.dict("sys.modules", {"rich.text": None}):
        ...
        from coco_codes.command_line import onboarding_slides
...
def test_add_model_menu_non_tool_calling_confirm():
    ...
    with (
        patch("coco_codes.command_line.add_model_menu.Application") as mock_app_cls,
        patch("coco_codes.command_line.add_model_menu.set_awaiting_user_input"),
        patch("coco_codes.command_line.add_model_menu.safe_input", return_value="y"),
