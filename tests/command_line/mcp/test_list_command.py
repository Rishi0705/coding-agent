...
class TestListCommand:
    def test_generates_group_id(self, list_cmd):
        with patch("coding_agent.command_line.mcp.list_command.emit_info"):
            list_cmd.manager.list_servers.return_value = []
            list_cmd.execute([])  # no group_id

    def test_no_servers(self, list_cmd):
        list_cmd.manager.list_servers.return_value = []
        with patch("coding_agent.command_line.mcp.list_command.emit_info") as mock_emit:
    ...
    def test_exception(self, list_cmd):
        ...
        with patch("coding_agent.command_line.mcp.list_command.emit_error") as mock_err:
            list_cmd.execute([], group_id="g1")
