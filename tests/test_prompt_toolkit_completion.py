...
def test_set_completer_excludes_auth_token(monkeypatch):
    # Ensure 'auth_token' is a config key but SetCompleter doesn't offer it
    monkeypatch.setattr(
        "coding_agent.command_line.prompt_toolkit_completion.get_config_keys",
        lambda: ["auth_token", "user_name", "temp_dir"],
    )
    monkeypatch.setattr(
        "coding_agent.command_line.prompt_toolkit_completion.get_value",
        lambda key: "sensitive_token_value" if key == "auth_token" else "normal_value",
    )
    completer = SetCompleter()

    # Test with full "auth_token" typed
    doc = Document(text="/set auth_token", cursor_position=len("/set auth_token"))
    completions = list(completer.get_completions(doc, None))
    assert completions == [], (
        "SetCompleter should not complete for 'auth_token' key directly"
    )

    # Test with partial "puppy" that would match "auth_token"
    doc = Document(text="/set puppy", cursor_position=len("/set puppy"))
    completions = list(completer.get_completions(doc, None))
    assert completions == [], (
        "SetCompleter should not complete for 'auth_token' key even partially"
    )
...
def test_set_completer_no_match(monkeypatch):
    monkeypatch.setattr("coding_agent.config.get_config_keys", lambda: ["actual_key"])
    completer = SetCompleter()
...
@pytest.mark.asyncio
@patch("coding_agent.command_line.prompt_toolkit_completion.PromptSession")
@patch("coding_agent.command_line.prompt_toolkit_completion.FileHistory")
@patch("coding_agent.command_line.prompt_toolkit_completion.merge_completers")
...
@pytest.mark.asyncio
@patch("coding_agent.command_line.prompt_toolkit_completion.PromptSession")
@patch("coding_agent.command_line.prompt_toolkit_completion.SafeFileHistory")
