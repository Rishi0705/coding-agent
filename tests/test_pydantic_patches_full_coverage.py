...
class TestGetCodingAgentVersion:
    def test_returns_version(self):
        from coding_agent.pydantic_patches import _get_coding_agent_version

        version = _get_coding_agent_version()
    ...
    def test_returns_dev_on_error(self):
        with patch("importlib.metadata.version", side_effect=Exception("nope")):
            ...
            result = _get_coding_agent_version()
            assert result == "0.0.0-dev"


class TestPatchUserAgent:
    def test_patch_user_agent_sets_function(self):
        from coding_agent.pydantic_patches import patch_user_agent

        patch_user_agent()
        # After patching, calling the function should return Coding-Agent/...
        import pydantic_ai.models as pydantic_models

        ua = pydantic_models.get_user_agent()
        assert "Coding-Agent" in ua or "KimiCLI" in ua
    ...
    def test_kimi_model_returns_kimi_ua(self):
        ...
        with patch("coding_agent.config.get_global_model_name", return_value="kimi-test"):
            ua = pydantic_models.get_user_agent()
            assert ua == "KimiCLI/0.63"

    def test_non_kimi_returns_coding_agent_ua(self):
        from coding_agent.pydantic_patches import patch_user_agent

        patch_user_agent()
        import pydantic_ai.models as pydantic_models

        with patch("coding_agent.config.get_global_model_name", return_value="gpt-4"):
            ua = pydantic_models.get_user_agent()
            assert "Coding-Agent" in ua
    ...
    def test_get_model_name_exception(self):
        ...
        with patch("coding_agent.config.get_global_model_name", side_effect=Exception):
            ua = pydantic_models.get_user_agent()
            assert "Coding-Agent" in ua
...
class TestPatchMessageHistoryCleaning:
    def test_patches_clean_message_history(self):
        from coding_agent.pydantic_patches import patch_message_history_cleaning
...
class TestPatchProcessMessageHistory:
    ...
    async def test_patched_process_runs_processors(self):
        from coding_agent.pydantic_patches import patch_process_message_history
...
class TestPatchToolCallJsonRepair:
    def test_patches_tool_manager(self):
        from coding_agent.pydantic_patches import patch_tool_call_json_repair
...
class TestPatchToolCallCallbacks:
    def test_patches_tool_manager(self):
        from coding_agent.pydantic_patches import patch_tool_call_callbacks
