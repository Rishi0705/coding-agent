...
class TestPuppyTokens:
    ...
    def test_get_auth_token_returns_value_if_set(self, mock_config_paths):
        """Test getting puppy token when it's set."""
        mock_cfg_dir, mock_cfg_file, _ = mock_config_paths

        config = configparser.ConfigParser()
        config["puppy"] = {"auth_token": "secret-token-123"}
        os.makedirs(mock_cfg_dir, exist_ok=True)
        with open(mock_cfg_file, "w") as f:
            config.write(f)

        result = cp_config.get_auth_token()
        assert result == "secret-token-123"

    def test_get_auth_token_returns_none_if_not_set(self, mock_config_paths):
    ...
    def test_set_auth_token_persists_value(self, mock_config_paths):
        ...
        cp_config.set_auth_token("new-token-456")

        saved_config = configparser.ConfigParser()
        saved_config.read(mock_cfg_file)
        assert saved_config["puppy"]["auth_token"] == "new-token-456"
