...
class TestAgentManagerErrors:
    ...
    def test_load_agent_invalid_name(self, mock_discover):
        ...
        with patch("coding_agent.agents.agent_manager._AGENT_REGISTRY", {}):
            with pytest.raises(
    ...
    def test_load_agent_fallback_behavior(self, mock_discover):
        """Test load_agent fallback to coding-agent when requested agent not found."""
        mock_discover.return_value = None

        # Mock registry with only coding-agent available
        mock_agent_class = MagicMock(spec=BaseAgent)
        mock_agent_class.return_value.name = "coding-agent"

        with patch(
            "coding_agent.agents.agent_manager._AGENT_REGISTRY",
            {"coding-agent": mock_agent_class},
        ):
            # Should fallback to coding-agent instead of raising error
    ...
    def test_load_agent_corrupted_registry_entry(self, mock_discover):
        ...
        with patch(
            "coding_agent.agents.agent_manager._AGENT_REGISTRY", {"bad-agent": 12345}
        ):
    ...
    @patch("coding_agent.agents.agent_manager._CURRENT_AGENT", None)
    ...
    def test_load_agent_case_sensitivity(self, mock_discover):
        ...
        with patch(
            ...
            with pytest.raises(ValueError, match="Agent 'coding-agent' not found"):
                load_agent("coding-agent")

            # Exact case should work
            result = load_agent("Coding-Agent")
            assert result is not None
            mock_agent_class.assert_called_once()
