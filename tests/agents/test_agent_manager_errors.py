...
class TestAgentManagerErrors:
    ...
    def test_load_agent_invalid_name(self, mock_discover):
        ...
        with patch("coco_codes.agents.agent_manager._AGENT_REGISTRY", {}):
            with pytest.raises(
    ...
    def test_load_agent_fallback_behavior(self, mock_discover):
        """Test load_agent fallback to coco-codes when requested agent not found."""
        mock_discover.return_value = None

        # Mock registry with only coco-codes available
        mock_agent_class = MagicMock(spec=BaseAgent)
        mock_agent_class.return_value.name = "coco-codes"

        with patch(
            "coco_codes.agents.agent_manager._AGENT_REGISTRY",
            {"coco-codes": mock_agent_class},
        ):
            # Should fallback to coco-codes instead of raising error
    ...
    def test_load_agent_corrupted_registry_entry(self, mock_discover):
        ...
        with patch(
            "coco_codes.agents.agent_manager._AGENT_REGISTRY", {"bad-agent": 12345}
        ):
    ...
    @patch("coco_codes.agents.agent_manager._CURRENT_AGENT", None)
    ...
    def test_load_agent_case_sensitivity(self, mock_discover):
        ...
        with patch(
            ...
            with pytest.raises(ValueError, match="Agent 'coco-codes' not found"):
                load_agent("coco-codes")

            # Exact case should work
            result = load_agent("Coco-Codes")
            assert result is not None
            mock_agent_class.assert_called_once()
