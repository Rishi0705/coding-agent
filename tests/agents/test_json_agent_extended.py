...
class TestDiscoverJsonAgents:
    ...
    def test_discover_valid_agents(self, tmp_path, monkeypatch):
        ...
        # Use a temp directory without .coding_agent to isolate from project directory
        isolated_dir = tmp_path / "isolated"
        isolated_dir.mkdir()
        monkeypatch.chdir(isolated_dir)

        # Mock only the user agents directory to point to our tmp_path
        with patch("coding_agent.config.get_user_agents_directory") as mock_get_user_dir:
    ...
    def test_discover_skip_invalid_agents(self, tmp_path, monkeypatch):
        ...
        # Change to isolated directory to avoid project .coding_agent
        isolated_dir = tmp_path / "isolated"
        isolated_dir.mkdir()
        monkeypatch.chdir(isolated_dir)
...
class TestDiscoverProjectJsonAgents:
    ...
    def test_discover_project_agents(self, tmp_path):
        ...
        project_dir = tmp_path / "project" / ".coding_agent" / "agents"
        ...
        with (
            ...
            patch(
                "coding_agent.config.get_project_agents_directory",
