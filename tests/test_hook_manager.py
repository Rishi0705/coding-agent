...
class TestLoadHooksConfig:
    def test_loads_hooks_key(self, tmp_settings, monkeypatch):
        from coco_codes.plugins.hook_manager.config import load_hooks_config
...
class TestSaveHooksConfig:
    def test_round_trip(self, tmp_settings, monkeypatch):
        from coco_codes.plugins.hook_manager.config import (
            load_hooks_config,
            save_hooks_config,
        )
    ...
    def test_preserves_other_top_level_keys(self, tmp_path, monkeypatch):
        from coco_codes.plugins.hook_manager.config import save_hooks_config
...
class TestHookEntry:
    def _make(self, **kwargs):
        from coco_codes.plugins.hook_manager.config import HookEntry
...
class TestFlattenHooks:
    def test_correct_count(self):
        from coco_codes.plugins.hook_manager.config import flatten_hooks
...
class TestToggleHookEnabled:
    def test_enables_disabled_hook(self):
        from coco_codes.plugins.hook_manager.config import toggle_hook_enabled
...
class TestDeleteHook:
    def test_removes_hook(self):
        from coco_codes.plugins.hook_manager.config import delete_hook
...
class TestWrap:
    def test_short_string_single_line(self):
        from coco_codes.plugins.hook_manager.hooks_menu import _wrap
...
class TestHooksMenuDataMethods:
    def _make_menu(self, tmp_settings, monkeypatch):
        ...
        from coco_codes.plugins.hook_manager.hooks_menu import HooksMenu

        monkeypatch.chdir(tmp_settings.parent.parent)
        # Mock _load_global_hooks_config to return empty dict for test isolation
        with patch(
            "coco_codes.plugins.hook_manager.config._load_global_hooks_config",
            return_value={},
        ):
            return HooksMenu()
...
class TestHandleHooksCommand:
    def _call(self, command: str, name: str = "hooks"):
        from coco_codes.plugins.hook_manager.register_callbacks import (
    ...
    def test_enable_subcommand(self, tmp_settings, monkeypatch):
        monkeypatch.chdir(tmp_settings.parent.parent)
        with patch("coco_codes.messaging.emit_success") as mock_ok:
            with patch(
                "coco_codes.plugins.hook_manager.config._load_global_hooks_config",
                return_value={},
            ):
    ...
    def test_unknown_subcommand_returns_true_with_error(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        with patch("coco_codes.messaging.emit_error") as mock_err:
            with patch(
                "coco_codes.plugins.hook_manager.config._load_global_hooks_config",
                return_value={},
            ):
...
class TestPluginRegistration:
    def test_custom_command_callback_registered(self):
        ...
        import coco_codes.plugins.hook_manager.register_callbacks as rcb
