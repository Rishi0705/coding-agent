...
class TestConfigExtendedPart1:
    ...
    def test_get_value_with_existing_key(self, mock_config_file):
        ...
        result = get_value("assistant_name")
    ...
    def test_set_value_existing_key(self, mock_config_file):
        ...
        original = get_value("assistant_name")
        assert original == "TestPuppy"

        # Update it
        set_config_value("assistant_name", "UpdatedPuppy")
    ...
    def test_get_allow_recursion_default(self, temp_config_dir):
        ...
        config = configparser.ConfigParser()
        config[DEFAULT_SECTION] = {"assistant_name": "Test"}
        with open(config_file, "w") as f:
            config.write(f)
    ...
    def test_get_assistant_name_default(self, temp_config_dir):
        """Test get_assistant_name returns default when not set"""
        temp_dir, config_file = temp_config_dir

        # Create config without assistant_name
        config = configparser.ConfigParser()
        config[DEFAULT_SECTION] = {}
        with open(config_file, "w") as f:
            config.write(f)
