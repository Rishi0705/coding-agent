...
import coding_agent.terminal_utils  # noqa: E402
from coding_agent.terminal_utils import (  # noqa: E402
...
class TestCrossPlatformReset:
    ...
    @patch("platform.system")
    @patch("coding_agent.terminal_utils.reset_windows_terminal_full")
...
class TestWindowsCtrlCDisable:
    ...
    @patch("platform.system")
    @patch("ctypes.windll.kernel32")
    @patch("coding_agent.terminal_utils._original_ctrl_handler", None)
...
class TestWindowsCtrlCEnable:
    ...
    @patch("platform.system")
    @patch("coding_agent.terminal_utils._original_ctrl_handler", 0x0007)
    @patch("ctypes.windll.kernel32")
    ...
    def test_enable_ctrl_c_no_restore_needed(self, mock_platform):
        ...
        with patch("coding_agent.terminal_utils._original_ctrl_handler", None):
            result = enable_windows_ctrl_c()
...
class TestSetKeepCtrlCDisabled:
    ...
    def test_set_keep_ctrl_c_disabled_true(self):
        ...
        assert coding_agent.terminal_utils._keep_ctrl_c_disabled is True

    def test_set_keep_ctrl_c_disabled_false(self):
        """Test setting keep Ctrl+C disabled to False."""
        set_keep_ctrl_c_disabled(False)

        assert coding_agent.terminal_utils._keep_ctrl_c_disabled is False


class TestEnsureCtrlCDisabled:
    """Test ensure_ctrl_c_disabled function."""

    @patch("coding_agent.terminal_utils._keep_ctrl_c_disabled", False)
    ...
    @patch("platform.system")
    @patch("coding_agent.terminal_utils._keep_ctrl_c_disabled", True)
...
class TestPrintTruecolorWarning:
    ...
    def test_no_warning_when_truecolor_supported(self):
        ...
        with patch("coding_agent.terminal_utils.detect_truecolor_support") as mock_detect:
...
class TestEdgeCasesAndIntegration:
    ...
    def test_reset_routing_based_on_platform(self):
        ...
        with patch("platform.system", return_value="Windows"):
            with patch(
                "coding_agent.terminal_utils.reset_windows_terminal_full"
            ) as mock_win:
                reset_terminal()
        ...
        with patch("platform.system", return_value="Linux"):
            with patch("coding_agent.terminal_utils.reset_unix_terminal") as mock_unix:
                reset_terminal()
