"""Full coverage tests for coding_agent/tools/common.py.
...
class TestGetLexerForExtension:
    ...
    def test_no_pygments(self):
        import coding_agent.tools.common as mod
    ...
    def test_get_lexer_by_name_exception(self):
        ...
        from coding_agent.tools.common import PYGMENTS_AVAILABLE, _get_lexer_for_extension

        if not PYGMENTS_AVAILABLE:
            pytest.skip("Pygments not available")
        with patch(
            "coding_agent.tools.common.get_lexer_by_name",
...
class TestGetTokenColor:
    ...
    def test_keyword_token(self):
        from coding_agent.tools.common import PYGMENTS_AVAILABLE, _get_token_color
...
class TestHighlightCodeLine:
    ...
    def test_with_lexer_none(self):
        from coding_agent.tools.common import _highlight_code_line
...
class TestArrowSelectAsync:
    ...
    async def test_basic_selection(self):
        ...
        with patch("coding_agent.tools.common.Application") as MockApp:
            ...
            with patch("coding_agent.tools.common.arrow_select_async") as mock_sel:
    ...
    async def test_cancel_raises_keyboard_interrupt(self):
        from coding_agent.tools.common import arrow_select_async
...
class TestArrowSelect:
    def test_raises_in_async_context(self):
        ...
        from coding_agent.tools.common import arrow_select
...
class TestGetUserApproval:
    def test_approve(self):
        ...
        with patch("coding_agent.tools.common.arrow_select", return_value="✓ Approve"):
            with patch("coding_agent.tools.common.Console"):
                with patch("coding_agent.tools.command_runner.set_awaiting_user_input"):
                    with patch("coding_agent.tools.common.emit_info"):
                        with patch("coding_agent.tools.common.emit_success"):
                            confirmed, feedback = get_user_approval(
                                "Test", "content", assistant_name="Biscuit"
                            )
    ...
    def test_reject(self):
        ...
        with patch("coding_agent.tools.common.arrow_select", return_value="✗ Reject"):
            with patch("coding_agent.tools.common.Console"):
                with patch("coding_agent.tools.command_runner.set_awaiting_user_input"):
                    with patch("coding_agent.tools.common.emit_info"):
                        with patch("coding_agent.tools.common.emit_error"):
                            confirmed, feedback = get_user_approval(
                                "Test", "content", assistant_name="Biscuit"
                            )
    ...
    def test_reject_with_feedback(self):
        ...
        with patch(
            ...
            with patch("coding_agent.tools.common.Prompt") as MockPrompt:
                ...
                with patch("coding_agent.tools.common.Console"):
                    with patch(
                        ...
                        with patch("coding_agent.tools.common.emit_info"):
                            with patch("coding_agent.tools.common.emit_error"):
                                with patch("coding_agent.tools.common.emit_warning"):
                                    confirmed, feedback = get_user_approval(
                                        "Test", "content", assistant_name="Biscuit"
                                    )
        assert confirmed is False
    ...
    def test_reject_with_empty_feedback(self):
        ...
        with patch("coding_agent.tools.common.arrow_select", return_value="💬 feedback"):
            with patch("coding_agent.tools.common.Prompt") as MockPrompt:
    ...
    def test_keyboard_interrupt(self):
        ...
        with patch(
            "coding_agent.tools.common.arrow_select", side_effect=KeyboardInterrupt
        ):
            with patch("coding_agent.tools.common.Console"):
                with patch("coding_agent.tools.command_runner.set_awaiting_user_input"):
                    with patch("coding_agent.tools.common.emit_info"):
    ...
    def test_eof_error(self):
        ...
        with patch("coding_agent.tools.common.arrow_select", side_effect=EOFError):
            with patch("coding_agent.tools.common.Console"):
                with patch("coding_agent.tools.command_runner.set_awaiting_user_input"):
                    with patch("coding_agent.tools.common.emit_info"):
                        with patch("coding_agent.tools.common.emit_error"):
            ...
            with patch("coding_agent.tools.common.Console"):
                with patch("coding_agent.tools.command_runner.set_awaiting_user_input"):
                    with patch("coding_agent.tools.common.emit_info"):
                        with patch("coding_agent.tools.common.emit_success"):
                            confirmed, _ = get_user_approval(
                                "Test", Text("rich content"), assistant_name="Biscuit"
                            )
        assert confirmed is True
            ...
            with patch("coding_agent.tools.common.Console"):
                with patch("coding_agent.tools.command_runner.set_awaiting_user_input"):
                    with patch("coding_agent.tools.common.emit_info"):
                        with patch("coding_agent.tools.common.emit_success"):
                            confirmed, _ = await get_user_approval_async(
                                "Test", Text("rich"), assistant_name="Biscuit"
                            )
        assert confirmed is True

    @pytest.mark.asyncio
    async def test_default_assistant_name(self):
        from coding_agent.tools.common import get_user_approval_async
...
class TestModuleConstants:
    def test_extension_to_lexer_name(self):
        from coding_agent.tools.common import EXTENSION_TO_LEXER_NAME

        assert ".py" in EXTENSION_TO_LEXER_NAME
        assert EXTENSION_TO_LEXER_NAME[".py"] == "python"

    def test_token_colors_dict(self):
        from coding_agent.tools.common import PYGMENTS_AVAILABLE, TOKEN_COLORS
