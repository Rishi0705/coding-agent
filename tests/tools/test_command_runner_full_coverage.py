"""Full coverage tests for coding_agent/tools/command_runner.py.
...
class TestTruncateLine:
    def test_short_line(self):
        from coding_agent.tools.command_runner import _truncate_line

        assert _truncate_line("hello") == "hello"

    def test_exact_limit(self):
        from coding_agent.tools.command_runner import MAX_LINE_LENGTH, _truncate_line
...
class TestProcessRegistration:
    def test_register_and_unregister(self):
        from coding_agent.tools.command_runner import (
            _RUNNING_PROCESSES,
    ...
    def test_unregister_nonexistent(self):
        from coding_agent.tools.command_runner import _unregister_process
...
class TestKillProcessGroup:
    def test_posix_kill(self):
        from coding_agent.tools.command_runner import _kill_process_group
    ...
    def test_exception_in_kill(self):
        ...
        with patch("coding_agent.tools.command_runner.emit_error"):
            # Should not raise
            _kill_process_group(proc)
...
class TestKillAllRunningShellProcesses:
    def test_kills_processes(self):
        ...
        with patch("coding_agent.tools.command_runner._kill_process_group"):
...
class TestAwaitingUserInput:
    def test_set_and_check(self):
        ...
        with patch("coding_agent.tools.command_runner.pause_all_spinners", create=True):
            pass
...
class TestSpawnCtrlXKeyListener:
    def test_no_tty(self):
        from coding_agent.tools.command_runner import _spawn_ctrl_x_key_listener
...
class TestKeyboardContext:
    def test_acquire_release(self):
        import coding_agent.tools.command_runner as mod
...
class TestShellCommandKeyboardContext:
    def test_context_manager(self):
        from coding_agent.tools.command_runner import _shell_command_keyboard_context

        with patch(
            "coding_agent.tools.command_runner._spawn_ctrl_x_key_listener",
            return_value=None,
        ):
...
class TestRunShellCommandStreaming:
    def test_successful_command(self):
        ...
        with patch("coding_agent.tools.command_runner.emit_shell_line"):
            with patch("coding_agent.tools.command_runner.get_message_bus") as mock_bus:
    ...
    def test_silent_mode(self):
        ...
        with patch("coding_agent.tools.command_runner.emit_shell_line") as mock_emit:
            result = run_shell_command_streaming(
...
class TestRunShellCommand:
    ...
    async def test_empty_command(self):
        from coding_agent.tools.command_runner import run_shell_command

        ctx = MagicMock(spec=RunContext)
        with patch(
            "coding_agent.callbacks.on_run_shell_command",
    ...
    async def test_yolo_mode_executes(self):
        ...
        with patch(
            ...
            with patch("coding_agent.config.get_yolo_mode", return_value=True):
                with patch(
                    "coding_agent.tools.command_runner.is_subagent", return_value=False
                ):
                    with patch(
                        "coding_agent.tools.command_runner._execute_shell_command",
                        new_callable=AsyncMock,
    ...
    async def test_subagent_runs_silently(self):
        ...
        with patch(
            ...
            with patch("coding_agent.config.get_yolo_mode", return_value=False):
                with patch(
                    "coding_agent.tools.command_runner.is_subagent", return_value=True
                ):
                    with patch(
                        "coding_agent.tools.command_runner._execute_shell_command",
                        new_callable=AsyncMock,
    ...
    async def test_background_command(self):
        ...
        with patch(
            ...
            with patch("subprocess.Popen") as MockPopen:
                ...
                with patch(
                    "coding_agent.tools.command_runner.get_message_bus"
                ) as mock_bus:
                    mock_bus.return_value = MagicMock()
                    with patch("coding_agent.tools.command_runner.emit_info"):
            ...
            with patch("coding_agent.config.get_yolo_mode", return_value=False):
                with patch(
                    ...
                    with patch("sys.stdin") as mock_stdin:
                        ...
                        with patch(
                            "coding_agent.tools.command_runner.get_user_approval_async",
                            new_callable=AsyncMock,
                            return_value=(False, None),
                        ):
                            with patch(
                                "coding_agent.config.get_assistant_name", return_value="buddy"
                            ):
                                result = await run_shell_command(
    ...
    async def test_confirmation_lock_contention(self):
        ...
        try:
            with patch(
                ...
                with patch("coding_agent.config.get_yolo_mode", return_value=False):
                    with patch(
                        "coding_agent.tools.command_runner.is_subagent",
                        return_value=False,
                    ):
                        with patch("sys.stdin") as mock_stdin:
                            mock_stdin.isatty.return_value = True
...
class TestExecuteShellCommand:
    ...
    async def test_executes(self):
        ...
        with patch("coding_agent.tools.command_runner.get_message_bus") as mock_bus:
            mock_bus.return_value = MagicMock()
            with patch("coding_agent.messaging.spinner.pause_all_spinners"):
                with patch("coding_agent.messaging.spinner.resume_all_spinners"):
                    with patch(
                        "coding_agent.tools.command_runner._acquire_keyboard_context"
                    ):
                        with patch(
                            "coding_agent.tools.command_runner._release_keyboard_context"
                        ):
                            with patch(
                                "coding_agent.tools.command_runner._run_command_inner",
                                new_callable=AsyncMock,
...
class TestRunCommandInner:
    ...
    async def test_exception(self):
        from coding_agent.tools.command_runner import _run_command_inner

        with patch(
            "coding_agent.tools.command_runner._run_command_sync",
...
class TestReasoningOutput:
    def test_default(self):
        from coding_agent.tools.command_runner import ReasoningOutput
...
class TestSignalHandlers:
    def test_handle_ctrl_x_press(self):
        from coding_agent.tools.command_runner import _handle_ctrl_x_press

        with patch("coding_agent.tools.command_runner.emit_warning"):
            with patch(
                "coding_agent.tools.command_runner.kill_all_running_shell_processes",
                return_value=0,
            ):
                _handle_ctrl_x_press()

    def test_shell_sigint_handler(self):
        from coding_agent.tools.command_runner import _shell_sigint_handler
...
class TestRunCommandSync:
    def test_basic_sync(self):
        from coding_agent.tools.command_runner import _run_command_sync
