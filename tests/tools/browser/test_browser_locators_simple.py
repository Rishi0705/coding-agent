...
from coco_codes.tools.browser.browser_locators import find_by_role
...
async def test_find_by_role_basic():
    ...
    with patch(
        "coco_codes.tools.browser.browser_locators.get_session_browser_manager",
        return_value=mock_manager,
    ):
        with patch(
            "coco_codes.tools.browser.browser_locators.emit_info"
...
async def test_find_by_role_no_page():
    ...
    with patch(
        ...
        with patch("coco_codes.tools.browser.browser_locators.emit_info"):
            result = await find_by_role("button")
