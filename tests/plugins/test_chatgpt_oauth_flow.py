...
from coco_codes.plugins.chatgpt_oauth.oauth_flow import (
    AuthBundle,
    TokenData,
    _CallbackHandler,
    _OAuthServer,
...
def mock_context():
    """Mock OAuth context."""
    from coco_codes.plugins.chatgpt_oauth.utils import OAuthContext
...
class TestOAuthServer:
    ...
    def test_exchange_code_success(self, mock_post, mock_context, mock_tokens_data):
        ...
        # Mock JWT parsing
        with patch(
            "coco_codes.plugins.chatgpt_oauth.oauth_flow.parse_jwt_claims"
        ) as mock_parse:
...
class TestCallbackHandler:
    ...
    def callback_handler(self, mock_server):
        ...
        with patch(
            "coco_codes.plugins.chatgpt_oauth.oauth_flow._CallbackHandler.handle_one_request"
        ):
    ...
    @patch("coco_codes.plugins.chatgpt_oauth.oauth_flow.save_tokens")
    ...
    def test_log_message_verbose_mode(self, callback_handler):
        ...
        with patch(
            "coco_codes.plugins.chatgpt_oauth.oauth_flow.BaseHTTPRequestHandler.log_message"
        ) as mock_log:
...
class TestRunOAuthFlow:
    ...
    @patch("coco_codes.plugins.chatgpt_oauth.oauth_flow.load_stored_tokens")
    @patch("coco_codes.plugins.chatgpt_oauth.oauth_flow._OAuthServer")
    @patch("coco_codes.plugins.chatgpt_oauth.oauth_flow.emit_warning")
    @patch("coco_codes.plugins.chatgpt_oauth.oauth_flow.emit_info")
    ...
    @patch("coco_codes.plugins.chatgpt_oauth.oauth_flow.load_stored_tokens")
    @patch("coco_codes.plugins.chatgpt_oauth.oauth_flow._OAuthServer")
    @patch("coco_codes.plugins.chatgpt_oauth.oauth_flow.emit_error")
    @patch("coco_codes.plugins.chatgpt_oauth.oauth_flow.emit_info")
    ...
    @patch("coco_codes.plugins.chatgpt_oauth.oauth_flow._OAuthServer")
    @patch("coco_codes.plugins.chatgpt_oauth.oauth_flow.emit_info")
    @patch("coco_codes.plugins.chatgpt_oauth.oauth_flow.emit_success")
    def test_successful_oauth_flow(
        ...
        with patch(
            "coco_codes.plugins.chatgpt_oauth.oauth_flow.load_stored_tokens"
        ) as mock_reload:
            mock_reload.return_value = mock_tokens

            with patch(
                "coco_codes.plugins.chatgpt_oauth.oauth_flow.add_models_to_extra_config"
            ) as mock_add:
                mock_add.return_value = True

                with patch(
                    "coco_codes.plugins.chatgpt_oauth.utils.fetch_chatgpt_models",
    ...
    def test_authentication_timeout(
        ...
        with patch(
            "coco_codes.plugins.chatgpt_oauth.oauth_flow.CHATGPT_OAUTH_CONFIG",
            original_config,
        ):
            with patch("threading.Thread"):
                with patch("time.sleep"):  # Skip the timing loop
...
class TestShutdownAfterDelay:
    ...
    def test_shutdown_after_delay_calls_shutdown(self):
        ...
        with patch(
            "coco_codes.plugins.chatgpt_oauth.oauth_flow.threading.Thread",
            side_effect=capture_thread,
        ):
            with patch("coco_codes.plugins.chatgpt_oauth.oauth_flow.time.sleep"):
