...
Tests for coding_agent/plugins/claude_code_hooks/
...
Implementation targets:
  coding_agent/plugins/claude_code_hooks/config.py
  coding_agent/plugins/claude_code_hooks/register_callbacks.py
"""
...
class TestGetHooksConfigPaths:
    def test_returns_list(self):
        from coding_agent.plugins.claude_code_hooks.config import get_hooks_config_paths
    ...
    def test_global_path_contains_coding_agent(self):
        from coding_agent.plugins.claude_code_hooks.config import get_hooks_config_paths

        paths = get_hooks_config_paths()
        # Second path is user-level (~/.coding_agent/hooks.json)
        assert ".coding_agent" in paths[1] and "hooks.json" in paths[1]
...
class TestLoadHooksConfigNoFiles:
    def test_returns_none_when_no_config_files(self):
        from coding_agent.plugins.claude_code_hooks.config import load_hooks_config

        with tempfile.TemporaryDirectory() as tmpdir:
            with patch(
                "coding_agent.plugins.claude_code_hooks.config.os.getcwd",
                return_value=tmpdir,
            ):
                with patch(
                    "coding_agent.plugins.claude_code_hooks.config.GLOBAL_HOOKS_FILE",
...
class TestOnPreToolCallHook:
    ...
    async def test_returns_none_when_no_engine(self):
        from coding_agent.plugins.claude_code_hooks import register_callbacks
    ...
    async def test_returns_none_when_hook_allows(self):
        from coding_agent.hook_engine.models import ProcessEventResult
        from coding_agent.plugins.claude_code_hooks import register_callbacks
    ...
    async def test_passes_tool_name_and_args_to_engine(self):
        from coding_agent.hook_engine.models import EventData, ProcessEventResult
        from coding_agent.plugins.claude_code_hooks import register_callbacks
...
class TestCallbackRegistration:
    def test_pre_tool_call_registered(self):
        ...
        from coding_agent.plugins.claude_code_hooks.register_callbacks import (
