...
Tests for coco_codes/plugins/claude_code_hooks/
...
Implementation targets:
  coco_codes/plugins/claude_code_hooks/config.py
  coco_codes/plugins/claude_code_hooks/register_callbacks.py
"""
...
class TestGetHooksConfigPaths:
    def test_returns_list(self):
        from coco_codes.plugins.claude_code_hooks.config import get_hooks_config_paths
    ...
    def test_global_path_contains_coco_codes(self):
        from coco_codes.plugins.claude_code_hooks.config import get_hooks_config_paths

        paths = get_hooks_config_paths()
        # Second path is user-level (~/.coco_codes/hooks.json)
        assert ".coco_codes" in paths[1] and "hooks.json" in paths[1]
...
class TestLoadHooksConfigNoFiles:
    def test_returns_none_when_no_config_files(self):
        from coco_codes.plugins.claude_code_hooks.config import load_hooks_config

        with tempfile.TemporaryDirectory() as tmpdir:
            with patch(
                "coco_codes.plugins.claude_code_hooks.config.os.getcwd",
                return_value=tmpdir,
            ):
                with patch(
                    "coco_codes.plugins.claude_code_hooks.config.GLOBAL_HOOKS_FILE",
...
class TestOnPreToolCallHook:
    ...
    async def test_returns_none_when_no_engine(self):
        from coco_codes.plugins.claude_code_hooks import register_callbacks
    ...
    async def test_returns_none_when_hook_allows(self):
        from coco_codes.hook_engine.models import ProcessEventResult
        from coco_codes.plugins.claude_code_hooks import register_callbacks
    ...
    async def test_passes_tool_name_and_args_to_engine(self):
        from coco_codes.hook_engine.models import EventData, ProcessEventResult
        from coco_codes.plugins.claude_code_hooks import register_callbacks
...
class TestCallbackRegistration:
    def test_pre_tool_call_registered(self):
        ...
        from coco_codes.plugins.claude_code_hooks.register_callbacks import (
