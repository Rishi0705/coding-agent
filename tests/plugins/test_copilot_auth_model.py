...
class TestCopilotAuth:
    ...
    def _make_auth(self, oauth_token: str = "ghp_fake123", host: str = "github.com"):
        ...
        from coco_codes.plugins.copilot_auth.utils import get_valid_session_token
    ...
    @patch("coco_codes.plugins.copilot_auth.utils.get_valid_session_token")
...
class TestCreateCopilotModel:
    ...
    MODULE = "coco_codes.plugins.copilot_auth.register_callbacks"

    @patch("coco_codes.plugins.copilot_auth.register_callbacks.get_token_for_host")
    def test_returns_none_when_no_token(self, mock_get_token):
        """Should return None and warn when no Copilot token is available."""
        mock_get_token.return_value = None

        from coco_codes.plugins.copilot_auth.register_callbacks import (
    ...
    @patch("coco_codes.plugins.copilot_auth.register_callbacks.get_valid_session_token")
    @patch("coco_codes.plugins.copilot_auth.register_callbacks.get_token_for_host")
    ...
    @patch(
        "coco_codes.plugins.copilot_auth.register_callbacks.get_api_endpoint_for_host"
    )
    @patch("coco_codes.plugins.copilot_auth.register_callbacks.get_valid_session_token")
    @patch("coco_codes.plugins.copilot_auth.register_callbacks.get_token_for_host")
