...
- Coco Codes configuration conversion
...
from coco_codes.models_dev_parser import (
    ModelInfo,
    ModelsDevRegistry,
    ProviderInfo,
    convert_to_coco_codes_config,
)
...
class TestModelsDevRegistryAPIFetching:
    ...
    @patch("coco_codes.models_dev_parser.httpx.Client")
...
class TestConvertToCocoCodesConfig:
    """Tests for Coco Codes configuration conversion."""
    ...
    def test_convert_basic(self, sample_data):
        """Test basic conversion to Coco Codes config."""
        provider, model = sample_data
        config = convert_to_coco_codes_config(model, provider)
...
class TestModelsDevRegistryUncoveredPaths:
    ...
    def test_load_data_bundled_json_decode_error(
        ...
        with patch("coco_codes.models_dev_parser.httpx.Client") as mock_client_class:
            mock_client = MagicMock()
            mock_client_class.return_value.__enter__.return_value = mock_client
...
class TestMainBlock:
    ...
    @patch("coco_codes.models_dev_parser.emit_error")
    @patch("coco_codes.models_dev_parser.emit_info")
    @patch("coco_codes.models_dev_parser.ModelsDevRegistry")
    def test_main_block_success(self, mock_registry_cls, mock_info, mock_error):
        ...
        with patch.dict("sys.modules", {}, clear=False):
            runpy.run_module("coco_codes.models_dev_parser", run_name="__main__")
    ...
    def test_main_block_file_not_found(self, mock_error):
        ...
        with patch(
            "coco_codes.models_dev_parser.ModelsDevRegistry",
