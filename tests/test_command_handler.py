...
def setup_messaging_mocks():
    ...
    mocks = {}
    patch_targets = [
        "coco_codes.messaging.emit_info",
        "coco_codes.messaging.emit_error",
...
def test_cd_show_lists_directories():
    ...
    try:
        with patch("coco_codes.command_line.utils.make_directory_table") as mock_table:
...
def test_set_config_value_equals():
    ...
    try:
        with (
            patch("coco_codes.config.set_config_value") as mock_set_cfg,
            patch(
                "coco_codes.config.get_config_keys", return_value=["pony", "rainbow"]
            ),
        ):
...
def test_set_config_only_key():
    ...
    try:
        with (
            patch("coco_codes.config.set_config_value") as mock_set_cfg,
            patch("coco_codes.config.get_config_keys", return_value=["key"]),
        ):
...
def test_show_status():
    ...
    try:
        with (
            ...
            patch("coco_codes.config.get_owner_name", return_value="Ivan"),
            patch("coco_codes.config.get_assistant_name", return_value="Biscuit"),
            patch("coco_codes.config.get_yolo_mode", return_value=True),
        ):
...
def test_set_no_args_prints_usage():
    ...
    try:
        with patch("coco_codes.config.get_config_keys", return_value=["foo", "bar"]):
...
def test_agent_switch_triggers_autosave_rotation():
    ...
    try:
        current_agent = SimpleNamespace(name="coco-codes", display_name="Coco Codes")
        ...
        with (
            ...
            patch(
                ...
                return_value={"coco-codes": "Coco Codes", "reviewer": "Reviewer"},
            ),
            patch(
            ...
            patch(
                "coco_codes.agents.set_current_agent",
                return_value=True,
            ) as mock_set,
        ):
...
def test_agent_switch_same_agent_skips_rotation():
    ...
    try:
        ...
        with (
            ...
            result = handle_command("/agent coco-codes")
            assert result is True
...
def test_tools_file_not_found():
    ...
    try:
        ...
        with patch("coco_codes.tools.tools_content.tools_content", "# Mock content"):
            result = handle_command("/tools")
            assert result is True
            mock_emit_info.assert_called_once()
            # Check that emit_info was called with a Markdown object
...
def test_tools_read_error():
    ...
    try:
        ...
        with patch(
            "coco_codes.command_line.core_commands.tools_content",
            "# Fallback content",
        ):
            result = handle_command("/tools")
            assert result is True
...
class TestRegistryIntegration:
    ...
    def test_unregistered_command_shows_warning(self):
        ...
        with patch("coco_codes.messaging.emit_warning") as mock_warn:
...
class TestSessionCommand:
    ...
    def test_session_show_current_id(self):
        ...
        with (
            patch("coco_codes.config.get_current_autosave_id", return_value="test-id"),
            patch(
                "coco_codes.config.get_current_autosave_session_name",
                return_value="test-session",
            ),
            patch("coco_codes.config.AUTOSAVE_DIR", "/tmp/autosave"),
            patch("coco_codes.messaging.emit_info") as mock_emit,
        ):
    ...
    def test_session_new_rotates(self):
        ...
        with (
            patch(
                "coco_codes.config.rotate_autosave_id", return_value="new-id"
            ) as mock_rotate,
            patch("coco_codes.messaging.emit_success") as mock_success,
        ):
    ...
    def test_session_alias_works(self):
        ...
        with (
            ...
            patch("coco_codes.config.AUTOSAVE_DIR", "/tmp"),
            patch("coco_codes.messaging.emit_info") as mock_emit,
        ):
...
class TestCompactCommand:
    ...
    def test_compact_with_history(self):
        ...
        with (
            ...
            patch("coco_codes.config.get_protected_token_count", return_value=1000),
    ...
    def test_compact_with_truncation_strategy(self):
        ...
        with (
            ...
            patch("coco_codes.config.get_protected_token_count", return_value=1000),
            patch(
                "coco_codes.agents._compaction.truncate",
...
class TestReasoningCommand:
    ...
    def test_reasoning_set_low(self):
        ...
        with (
            patch("coco_codes.config.set_openai_reasoning_effort") as mock_set,
            patch("coco_codes.config.get_openai_reasoning_effort", return_value="low"),
            patch(
                "coco_codes.agents.agent_manager.get_current_agent",
                return_value=mock_agent,
            ),
    ...
    def test_reasoning_invalid_level(self):
        ...
        with (
            ...
            patch("coco_codes.messaging.emit_error") as mock_error,
        ):
...
class TestGetCommandsHelp:
    ...
    def test_help_includes_registered_commands(self):
        ...
        from coco_codes.command_line.command_handler import get_commands_help
    ...
    def test_help_parses_tuple_format(self):
        ...
        with patch("coco_codes.callbacks.on_custom_command_help") as mock_callback:
...
def test_pin_model_command_case_insensitive_agent():
    ...
    with (
        ...
        patch("coco_codes.messaging.emit_success") as mock_emit_success,
        patch("coco_codes.messaging.emit_error") as mock_emit_error,
    ):
...
def test_pin_model_completion_case_insensitive_agent():
    ...
    from coco_codes.command_line.pin_command_completion import PinModelCompleter
...
def test_unpin_completion_case_insensitive_agent():
    ...
    from coco_codes.command_line.pin_command_completion import UnpinCompleter
