...
import coding_agent.status_display
from coding_agent.status_display import CURRENT_TOKEN_RATE, StatusDisplay
...
class TestStatusDisplay:
    ...
    def test_calculate_rate_with_previous_data(self, status_display):
        ...
        assert coding_agent.status_display.CURRENT_TOKEN_RATE == rate
    ...
    def test_calculate_rate_negative_rates_handled(self, status_display):
        ...
        rate = status_display._calculate_rate()
        assert rate >= 0
        assert coding_agent.status_display.CURRENT_TOKEN_RATE >= 0
    ...
    def test_update_rate_from_sse(self, status_display):
        ...
        assert coding_agent.status_display.CURRENT_TOKEN_RATE == 50
    ...
    def test_get_current_rate_static(self):
        ...
        try:
            coding_agent.status_display.CURRENT_TOKEN_RATE = 42.0
            assert StatusDisplay.get_current_rate() == 42.0
        finally:
            coding_agent.status_display.CURRENT_TOKEN_RATE = original_rate
    ...
    async def test_start(self, status_display):
        ...
        with patch("coding_agent.status_display.asyncio.create_task") as mock_create_task:
            mock_task = MagicMock()
    ...
    async def test_stop_after_start(self, status_display):
        ...
        with patch("coding_agent.status_display.asyncio.create_task") as mock_create_task:
            ...
            with patch("coding_agent.messaging.emit_info") as mock_emit_info:
                status_display.start()
                status_display.stop()
                ...
                assert status_display.start_time is None
                assert status_display.token_count == 0
                assert coding_agent.status_display.CURRENT_TOKEN_RATE == 0.0
    ...
    async def test_update_display_integration(self, status_display):
        ...
        with (
            patch("coding_agent.status_display.asyncio.create_task"),
            patch("coding_agent.status_display.Live") as mock_live,
            patch("coding_agent.messaging.emit_info") as mock_emit_info,
        ):
    ...
    def test_panel_styling(self, status_display):
        ...
        assert panel.title == "[bold blue]Coding Agent Status[/bold blue]"
