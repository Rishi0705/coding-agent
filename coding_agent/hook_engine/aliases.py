"""
Tool name alias registry — maps each AI provider's tool names to coding_agent's
...
Each provider block defines a dict[str, str] mapping:
    "<Provider tool name>" -> "<coding_agent internal tool name>"
...
CLAUDE_CODE_ALIASES: Dict[str, str] = {
    ...
    # NOTE: the tools below have no direct coding_agent equivalent yet.
...
GEMINI_ALIASES: Dict[str, str] = {
    # Add Gemini → coding_agent tool mappings here
}
...
CODEX_ALIASES: Dict[str, str] = {
    # Add Codex → coding_agent tool mappings here
}
...
SWARM_ALIASES: Dict[str, str] = {
    # Add Swarm → coding_agent tool mappings here
}
...
def resolve_internal_name(provider_tool_name: str) -> Optional[str]:
    """
    Return the coding_agent internal tool name for a given provider tool name,
