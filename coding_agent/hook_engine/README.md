...
```python
from coding_agent.hook_engine import HookEngine, EventData
...
```json
{
    "session_id": "coding-agent-session",
...
Hooks can be written using **either** the provider's tool name **or** coding_agent's
internal tool name — the matcher treats them as equivalent.

### Claude Code → coding_agent

| Claude Code (`matcher`) | coding_agent internal | Notes |
|-------------------------|---------------------|-------|
