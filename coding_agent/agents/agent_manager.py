...
from coding_agent.callbacks import on_agent_reload, on_register_agents
...
def _discover_agents(message_group_id: Optional[str] = None):
    ...
    import coding_agent.agents as agents_package
    ...
    for _, modname, _ in pkgutil.iter_modules(agents_package.__path__):
        ...
        try:
            # Import the module
            module = importlib.import_module(f"coding_agent.agents.{modname}")
    ...
    for _, subpkg_name, ispkg in pkgutil.iter_modules(agents_package.__path__):
        ...
        try:
            ...
            subpkg = importlib.import_module(f"coding_agent.agents.{subpkg_name}")
            ...
            for _, modname, _ in pkgutil.iter_modules(subpkg.__path__):
                ...
                try:
                    ...
                    module = importlib.import_module(
                        f"coding_agent.agents.{subpkg_name}.{modname}"
                    )
...
def get_current_agent_name() -> str:
    ...
    Returns:
        ...
        Priority: session agent > config default > 'coding-agent'.
    """
...
def load_agent(agent_name: str) -> BaseAgent:
    ...
    if agent_name not in _AGENT_REGISTRY:
        # Fallback to coding-agent if agent not found
        if "coding-agent" in _AGENT_REGISTRY:
