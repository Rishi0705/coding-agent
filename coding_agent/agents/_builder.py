...
entry point. Everything else in here (agent rules loading, MCP server loading,
...
hook; see :func:`coding_agent.callbacks.on_wrap_pydantic_agent`.
"""
...
from coding_agent.agents._compaction import make_history_processor
from coding_agent.agents._steer_processor import make_steer_history_processor
from coding_agent.agents.event_stream_handler import event_stream_handler
...
from coding_agent.mcp_ import get_mcp_manager
from coding_agent.messaging import emit_error, emit_info, emit_warning
from coding_agent.model_factory import ModelFactory, make_model_settings

_AGENT_RULE_FILES = ("AGENTS.md", "AGENT.md", "agents.md", "agent.md")
_CODING_AGENT_DIR = ".coding_agent"


def load_agent_rules() -> Optional[str]:
    """Load AGENT(S).md from global config dir and/or the current project dir.

    Global rules (``~/.coding_agent/AGENTS.md``) come first; project-local rules
    are appended, allowing projects to override/extend global ones.

    **Search order for project rules:**

    1. ``.coding_agent/AGENTS.md`` (preferred — keeps root clean)
    ...
    # Priority 1: Check .coding_agent/ directory (preferred location)
    coding_agent_dir = Path(_CODING_AGENT_DIR)
    if coding_agent_dir.is_dir():
        for name in _AGENT_RULE_FILES:
            candidate = coding_agent_dir / name
            if candidate.exists():
...
def _iter_autostart_targets(manager: Any, agent_name: str):
    ...
    """
    try:
        from coding_agent.mcp_.agent_bindings import get_bound_servers
...
def _assemble_instructions(agent: Any, resolved_model_name: str) -> str:
    """Compose full system prompt + agent rules + extended-thinking note."""
    from coding_agent.model_utils import prepare_prompt_for_model
...
def build_pydantic_agent(
    ...
    - ``agent._agent_rules = None`` (invalidates any cached rules)
