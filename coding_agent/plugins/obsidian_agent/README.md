...
The Obsidian Agent adds a specialized Coding Agent agent for working with Obsidian vaults through the official `obsidian` CLI.
...
Switch to the agent from Coding Agent and ask for an Obsidian task. Include a vault name or vault-relative path when relevant.
