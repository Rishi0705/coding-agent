...
> For **built-in commands**, see the built-in command files in `coding_agent/command_line/`.

## Overview

This plugin demonstrates how to create custom commands using Coding Agent's callback system.

**Important**: Custom commands use `register_callback()`, NOT `@register_command`.

## Command Types in Coding Agent

### 1. Built-in Commands (Core Functionality)
- Use `@register_command` decorator
- Located in `coding_agent/command_line/core_commands.py`, `session_commands.py`, `config_commands.py`
...
```
coding_agent/plugins/example_custom_command/
...
```bash
mkdir -p coding_agent/plugins/my_plugin
touch coding_agent/plugins/my_plugin/__init__.py
touch coding_agent/plugins/my_plugin/register_callbacks.py
```

### Step 2: Implement Callbacks

```python
# coding_agent/plugins/my_plugin/register_callbacks.py

from coding_agent.callbacks import register_callback
from coding_agent.messaging import emit_info, emit_success
...
```bash
# Restart Coding Agent to load the plugin
coding-agent
...
- **Don't modify global state**: Use Coding Agent's config system
...
from coding_agent.plugins.my_plugin.register_callbacks import _handle_custom_command
...
- `coding_agent/callbacks.py` - Callback system implementation
- `coding_agent/command_line/command_handler.py` - Command dispatcher
- `coding_agent/command_line/core_commands.py` - Example built-in commands
- `coding_agent/command_line/command_registry.py` - Registry system

## Questions?

If you're unsure whether to create a custom command or a built-in command:

- **Is it core Coding Agent functionality?** → Use `@register_command` (built-in)
