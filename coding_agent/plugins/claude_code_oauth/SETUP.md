...
This guide walks you through using the Claude Code OAuth plugin inside Coding Agent.

## Quick Start

1. Ensure the plugin files live under `coding_agent/plugins/claude_code_oauth/`
2. Restart Coding Agent so it loads the plugin
...
5. Tokens are stored locally at `~/.coding_agent/claude_code_oauth.json`
...
- Models appear in `~/.coding_agent/extra_models.json` with the `claude-code-` prefix
...
```bash
python coding_agent/plugins/claude_code_oauth/test_plugin.py
```
...
Enjoy using Claude Code with Coding Agent.
