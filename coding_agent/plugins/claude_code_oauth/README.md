...
This plugin adds OAuth authentication for Claude Code to Coding Agent, automatically importing available models into your configuration.
...
- **Token Management**: Secure storage of OAuth tokens in the Coding Agent config directory
...
After authentication, models will be added to `~/.coding_agent/extra_models.json`:
...
- **Token Storage**: Tokens are saved to `~/.coding_agent/claude_code_oauth.json` with `0o600` permissions
...
- Ensure the browser completed the redirect back to Coding Agent (no pop-up blockers)
...
- **Plugin Registration**: Custom command handlers wired into Coding Agent
