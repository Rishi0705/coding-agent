...
from coding_agent.plugins.chatgpt_oauth import config, utils
...
def test_config_paths():
    ...
    # XDG paths use "coding_agent" (without dot) in ~/.local/share or ~/.config
    assert "coding_agent" in str(token_path)

    config_dir = config.get_config_dir()
    # Default is ~/.coding_agent; XDG paths only used when XDG env vars are set
    assert config_dir.name in ("coding_agent", ".coding_agent")
...
@patch("coding_agent.plugins.chatgpt_oauth.utils.requests.post")
...
@patch("coding_agent.plugins.chatgpt_oauth.utils.requests.get")
