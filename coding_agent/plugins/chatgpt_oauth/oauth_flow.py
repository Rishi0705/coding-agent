    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if path == "/success":
            success_html = oauth_success_html(
                "ChatGPT",
                "You can now close this window and return to Coding Agent.",
            )
        ...
        if path != "/auth/callback":
            self._send_failure(404, "Callback endpoint not found for the oauth callback.")
            self._shutdown()
            return
        ...
        if save_tokens(tokens):
            ...
            self._send_failure(
                500, "Unable to persist auth file — a write error occurred."
            )
            self._shutdown()
        self._shutdown_after_delay(2.0)
