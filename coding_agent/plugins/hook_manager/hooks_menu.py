...
from both global (~/.coding_agent/hooks.json) and project (.claude/settings.json) sources.
...
class HooksMenu:
    ...
    def _render_list(self) -> List:
        ...
        if not self.entries:
            ...
            lines.append(("", "\n"))
            lines.append((_C_DIM, "  or ~/.coding_agent/hooks.json (global)"))
            lines.append(("", "\n\n"))
    ...
    def _render_detail(self) -> List:
        ...
        # Source indicator
        source_label = (
            "Global (~/.coding_agent/hooks.json)"
            if entry.source == "global"
