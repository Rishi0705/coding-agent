...
Storage lives at ``~/.coding_agent/token_ratios.json``, overridable via
the env var ``CODING_AGENT_TOKEN_RATIOS_PATH``.
"""
...
_TOKEN_RATIOS_PATH: Path = Path(
    os.path.expanduser(
        os.environ.get(
            "CODING_AGENT_TOKEN_RATIOS_PATH",
            str(Path.home() / ".coding_agent" / "token_ratios.json"),
        )
    )
)
