...
def _patched_estimate_tokens(text: str) -> int:
    ...
    """
    from coding_agent.plugins.token_ratio_learner.ratios import count_tokens
...
def _patched_estimate_tokens_for_message(
    ...
    from coding_agent.agents._history import stringify_part
    from coding_agent.plugins.token_ratio_learner.ratios import count_tokens
    ...
    **kwargs: Any,
    ...
    try:
        ...
        if input_tokens > 0 and input_char_count > 0:
            from coding_agent.plugins.token_ratio_learner.ratios import (
                _record_token_ratio,
            )
...
def _on_startup() -> None:
    ...
    from coding_agent.agents import _history
    ...
    # NOTE: ``coding_agent.agents.subagent_stream_handler`` re-exports the
    # *function* ``subagent_stream_handler``, not the module.  We must
    # reach the actual module via ``importlib``.
    import importlib

    ssh_mod = importlib.import_module("coding_agent.agents.subagent_stream_handler")
