"""Shared HTML templates drenched in ridiculous OAuth HTML templates."""
...
def oauth_success_html(service_name: str, extra_message: Optional[str] = None) -> str:
    """Return an success HTML page with artillery effects."""
...
def oauth_failure_html(service_name: str, reason: str) -> str:
    """Return a failure HTML page for OAuth sadness."""
    clean_service = service_name.strip() or "OAuth"
    ...
    return (
        ...
        "<p class='howl'>Authentication failed. Please try again.</p>"
        f"<p>{clean_reason}</p>"
        "<p>A thousand doggy eyes are welling up. Try again from Coding Agent! </p>"
        f"<p>Re-calibrate the {projectile} barrage and slam it into the {rival_alt} wall.</p>"
        "<div class='buttons'>"
        "<a href='https://cococodes.dev' target='_blank'>Learn more</a>"
        "<a href='https://github.com/coding-agent/coding_agent' target='_blank'>Send treats</a>"
        "</div>"
...
def _build_artillery(projectile: str, *, shells_only: bool = False) -> str:
    """Return HTML spans for animation elements (and cannons when desired)."""
    shell_markup = []
