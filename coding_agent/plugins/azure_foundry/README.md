# Azure AI Foundry Plugin for Coding Agent
...
4. **Python packages**: `azure-identity>=1.15.0` (installed with Coding Agent)
...
Models are stored in `~/.coding_agent/extra_models.json`:
...
4. **Integration**: Wraps the client in pydantic-ai's `AnthropicModel` for
   seamless integration with Coding Agent's agent system
...
```
coding_agent/plugins/azure_foundry/
...
This plugin follows Coding Agent's plugin architecture. Key callbacks:
...
Same as Coding Agent (see repository LICENSE file).
