...
*new line* below the agent/agent/model/cwd chrome, so long working-directory
...
* ``custom_command`` / ``custom_command_help`` — exposes ``/prompt_newline``
  for runtime on/off, persisted via ``coco.cfg``.
...
from coding_agent.plugins.prompt_newline.config import is_enabled, set_enabled
