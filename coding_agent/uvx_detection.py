"""Detect if coding-agent was launched via uvx on Windows.

This module provides utilities to detect the launch method of coding-agent,
specifically to handle signal differences when running via uvx on Windows.

On Windows, when launched via `uvx coding-agent`, Ctrl+C (SIGINT) gets captured
...
def is_launched_via_uvx() -> bool:
    """Detect if coding-agent was launched via uvx.
