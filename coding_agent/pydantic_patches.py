...
def _get_coding_agent_version() -> str:
    """Get the current coding-agent version."""
    try:
        return importlib.metadata.version("coding-agent")
    except Exception:
        return "0.0.0-dev"


def patch_user_agent() -> None:
    """Patch pydantic-ai's User-Agent to use Coco-Codes's version.
    ...
    - 'Coco-Codes/{version}' for all other models
    ...
    try:
        ...
        def _get_dynamic_user_agent() -> str:
            ...
            try:
                from coding_agent.config import get_global_model_name
