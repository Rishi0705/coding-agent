...
from coding_agent.session_storage import save_session


def _get_xdg_dir(env_var: str, fallback: str) -> str:
    """
    Get directory for coding_agent files, defaulting to ~/.coding_agent.

    XDG paths are only used when the corresponding environment variable
    is explicitly set by the user. Otherwise, we use the legacy ~/.coding_agent
    ...
    Returns:
        Path to the directory for coding_agent files
    """
    # Use XDG directory ONLY if environment variable is explicitly set
    xdg_base = os.getenv(env_var)
    if xdg_base:
        return os.path.join(xdg_base, "coding_agent")

    # Default to legacy ~/.coding_agent for all file types
    return os.path.join(os.path.expanduser("~"), ".coding_agent")
...
CONFIG_FILE = os.path.join(CONFIG_DIR, "coding_agent.cfg")
...
DEFAULT_SECTION = "coding_agent"
REQUIRED_KEYS = ["assistant_name", "owner_name"]
...
def ensure_config_exists():
    """
    Ensure that XDG directories and coding_agent.cfg exist, prompting if needed.
    ...
    if missing:
        ...
        sys.stdout.write("Let's set up Coding Agent.\n")
        sys.stdout.flush()
        for key in missing:
            if key == "assistant_name":
                val = input("What should we name the assistant? ").strip()
            elif key == "owner_name":
                val = input(
                    "What's your name (so Coding Agent knows its owner)? "
                ).strip()
            else:
...
def get_config_keys():
    """
    Returns the list of all config keys currently in coding_agent.cfg,
...
def load_mcp_server_configs():
    """
    Loads the MCP server configurations from XDG_CONFIG_HOME/coding_agent/mcp_servers.json.
    Returns a dict mapping names to their URL or config dict.
    If file does not exist, returns an empty dict.
    """
    from coding_agent.messaging.message_queue import emit_error
...
def get_global_model_name():
    """Return a valid model name for Coding Agent to use.
    ...
    2. Otherwise, look at ``model`` in *coding_agent.cfg*
...
def get_auth_token():
    """Returns the auth_token from config, or None if not set."""
    return get_value("auth_token")


def set_auth_token(token: str):
    """Sets the auth_token in the persistent config file."""
...
def get_user_agents_directory() -> str:
    ...
    Returns:
        Path to the user's Coding Agent agents directory.
    """
...
def get_project_agents_directory() -> Optional[str]:
    ...
    Looks for a .coding_agent/agents/ directory in the current working directory.
    ...
    """
    project_agents_dir = os.path.join(os.getcwd(), ".coding_agent", "agents")
    ...
    if not command_history_exists:
        try:
            ...
            old_history_file = os.path.join(
                os.path.expanduser("~"), ".coding_agent_history.txt"
            )
...
def get_yolo_mode():
    """
    Checks coding_agent.cfg for 'yolo_mode' (case-insensitive in value only).
...
def get_safety_permission_level():
    """
    Checks coding_agent.cfg for 'safety_permission_level' (case-insensitive in value only).
...
def get_mcp_disabled():
    """
    Checks coding_agent.cfg for 'disable_mcp' (case-insensitive in value only).
    Defaults to False if not set.
    Allowed values for ON: 1, '1', 'true', 'yes', 'on' (all case-insensitive for value).
    When enabled, Coding Agent will skip loading MCP servers entirely.
    """
    true_vals = {"1", "true", "yes", "on"}
...
def get_grep_output_verbose():
    """
    Checks coding_agent.cfg for 'grep_output_verbose' (case-insensitive in value only).
...
def get_auto_save_session() -> bool:
    """
    Checks coding_agent.cfg for 'auto_save_session' (case-insensitive in value only).
    Defaults to True if not set.
    Allowed values for ON: 1, '1', 'true', 'yes', 'on' (all case-insensitive for value).
    """
    true_vals = {"1", "true", "yes", "on"}
...
def get_suppress_thinking_messages() -> bool:
    """
    Checks coding_agent.cfg for 'suppress_thinking_messages' (case-insensitive in value only).
    Defaults to False if not set.
    Allowed values for ON: 1, '1', 'true', 'yes', 'on' (all case-insensitive for value).
...
def get_suppress_informational_messages() -> bool:
    """
    Checks coding_agent.cfg for 'suppress_informational_messages' (case-insensitive in value only).
    Defaults to False if not set.
    Allowed values for ON: 1, '1', 'true', 'yes', 'on' (all case-insensitive for value).
...
def get_api_key(key_name: str) -> str:
    """Get an API key from coding_agent.cfg.
...
def set_api_key(key_name: str, value: str):
    """Set an API key in coding_agent.cfg.
...
def load_api_keys_to_environment():
    """Load all API keys from .env and coding_agent.cfg into environment variables.

    Priority order:
    1. .env file (highest priority) - if present in current directory
    2. coding_agent.cfg - fallback if not in .env
    ...
    # Step 2: Load from coding_agent.cfg, but only if not already set
    # This ensures .env has priority over coding_agent.cfg
...
def get_default_agent() -> str:
    """
    Get the default agent name from coding_agent.cfg.

    Returns:
        str: The default agent name, or "coding-agent" if not set.
    """
    return get_value("default_agent") or "coding-agent"


def set_default_agent(agent_name: str) -> None:
    """
    Set the default agent name in coding_agent.cfg.
