"""
Models development API parser for Coding Agent.

This module provides functionality to parse and work with the models.dev API,
including provider and model information, search capabilities, and conversion to Coding Agent
...
# Provider type mapping for Coding Agent configuration
PROVIDER_TYPE_MAP = {
    "anthropic": "anthropic",
    "openai": "openai",
    "google": "gemini",
...
def convert_to_coding_agent_config(
    model: ModelInfo, provider: ProviderInfo
) -> Dict[str, Any]:
    """
    Convert a model and provider to Coding Agent configuration format.
    ...
    Returns:
        Dictionary in Coding Agent configuration format
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Example: Convert to Coding Agent config
        if providers and registry.get_models():
            provider = providers[0]
            models = registry.get_models(provider.id)
            if models:
                config = convert_to_coding_agent_config(models[0], provider)
