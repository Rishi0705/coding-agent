...
from coding_agent.pydantic_patches import apply_all_patches
...
from coding_agent import __version__, callbacks, plugins
from coding_agent.agents import get_current_agent
from coding_agent.command_line.attachments import parse_prompt_attachments
from coding_agent.command_line.clipboard import get_clipboard_manager
...
from coding_agent.messaging import emit_info
from coding_agent.terminal_utils import (
    reset_unix_terminal,
...
def _resume_session_from_path(raw_path: str) -> None:
    ...
    from coding_agent.messaging import emit_error, emit_success
    from coding_agent.session_storage import load_session
    ...
    try:
        from coding_agent.config import rotate_autosave_id
...
async def main():
    """Main async entry point for Coding Agent CLI."""
    parser = argparse.ArgumentParser(description="Coding Agent - A code generation agent")
    parser.add_argument(
        "--version",
        "-v",
        action="version",
    ...
    parser.add_argument(
        ...
        "-a",
        type=str,
        help="Specify which agent to use (e.g., --agent coding-agent)",
    )
    parser.add_argument(
        "--model",
        "-m",
    ...
    parser.add_argument(
        ...
        type=str,
        metavar="PATH",
        help="Resume a saved session from a .pkl file (e.g. ~/.coding_agent/contexts/foo.pkl)",
    )
    parser.add_argument(
    ...
    initialize_command_history_file()
    from coding_agent.messaging import emit_error, emit_system_message

    # Show the awesome Coding Agent logo when entering interactive mode
    # This happens when: no -p flag (prompt-only mode) is used
    # The logo should appear for both `coding-agent` and `coding-agent -i`
    if not args.prompt:
        try:
            import pyfiglet

            intro_lines = pyfiglet.figlet_format(
                "CODING AGENT", font="ansi_shadow"
            ).split("\n")
            ...
            emit_system_message("Coding Agent is Loading...")
    ...
    try:
        from coding_agent.uvx_detection import should_use_alternate_cancel_key
        ...
        if should_use_alternate_cancel_key():
            ...
            from coding_agent.terminal_utils import reset_windows_terminal_full
    ...
    # Load API keys from coding_agent.cfg into environment variables
    ...
    if args.model:
        from coding_agent.config import _validate_model_exists
...
async def interactive_mode(message_renderer, initial_command: str = None) -> None:
    ...
    from coding_agent.command_line.command_handler import handle_command

    display_console = message_renderer.console
    from coding_agent.messaging import emit_info, emit_system_message
    ...
    if initial_command:
        from coding_agent.agents import get_current_agent
        from coding_agent.messaging import emit_info, emit_success, emit_system_message
        ...
        try:
            ...
            try:
                from coding_agent.tools.command_runner import is_awaiting_user_input
            ...
            if response is not None:
                ...
                from coding_agent.messaging.messages import AgentResponseMessage
    ...
    try:
        ...
        from coding_agent.messaging import emit_warning
        ...
        try:
            ...
            from coding_agent.messaging import emit_success
            ...
            from coding_agent.messaging import emit_error, emit_warning
    ...
    try:
        from coding_agent.command_line.onboarding_wizard import should_show_onboarding

        if should_show_onboarding():
            import concurrent.futures

            from coding_agent.command_line.onboarding_wizard import run_onboarding_wizard
            from coding_agent.config import set_model_name
            from coding_agent.messaging import emit_info
            ...
            if result == "chatgpt":
                ...
                from coding_agent.plugins.chatgpt_oauth.oauth_flow import run_oauth_flow
    ...
    while True:
        ...
        try:
            ...
            try:
                ...
                try:
                    from coding_agent.terminal_utils import ensure_ctrl_c_disabled
            ...
            reset_windows_terminal_full()
            from coding_agent.callbacks import on_interactive_turn_cancel
            from coding_agent.messaging import emit_warning
            ...
            if command_result is True:
                ...
                if command_result == "__AUTOSAVE_LOAD__":
                    ...
                    try:
                        ...
                        if os.getenv("CODING_AGENT_NO_TUI") == "1":
                        ...
                        if use_interactive_picker:
                            ...
                            from coding_agent.command_line.autosave_menu import (
        ...
        if task.strip():
            ...
            try:
                ...
                turn_error = e
                from coding_agent.messaging.queue_console import get_queue_console

                get_queue_console().print_exception()

            # Auto-save session if enabled (moved outside the try block to avoid being swallowed)
            from coding_agent.config import auto_save_session_if_enabled
            ...
            from coding_agent.messaging import emit_system_message
...
async def run_prompt_with_attachments(
    ...
    *,
    ...
    from coding_agent.messaging import emit_system_message, emit_warning
    ...
    from coding_agent.agents.event_stream_handler import set_streaming_console
    ...
    if use_spinner and spinner_console is not None:
        from coding_agent.messaging.spinner import ConsoleSpinner
