...
try:
    ...
    NO_COLOR = bool(int(os.environ.get("CODING_AGENT_NO_COLOR", "0")))
...
def get_user_approval(
    ...
    Args:
        ...
        assistant_name: Name of the assistant (defaults to config value)
    ...
    # Show result BEFORE resuming spinners (clean ordering!)
    emit_info("")
    if not confirmed:
        if user_feedback:
            emit_error("Rejected with feedback!")
            emit_warning(f'Telling {assistant_name}: "{user_feedback}"')
        else:
            emit_error("Rejected.")
    else:
