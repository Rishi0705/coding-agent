"""Command handlers for Coding Agent - CONFIG commands.
...
from coding_agent.command_line.command_registry import register_command
from coding_agent.config import get_config_keys
...
def get_commands_help():
    ...
    from coding_agent.command_line.command_handler import get_commands_help as _gch
...
@register_command(
    name="show",
    description="Show config key-values",
    usage="/show",
    category="config",
)
def handle_show_command(command: str) -> bool:
    ...
    status_msg = f"""[bold bright_green]Coding Agent Status[/bold bright_green]
...
def handle_reasoning_command(command: str) -> bool:
    ...
    from coding_agent.messaging import emit_error, emit_success, emit_warning
    ...
    effort = tokens[1]
    try:
        from coding_agent.config import set_openai_reasoning_effort
    ...
    from coding_agent.config import get_openai_reasoning_effort
...
def handle_verbosity_command(command: str) -> bool:
    ...
    verbosity = tokens[1]
    try:
        from coding_agent.config import set_openai_verbosity
    ...
    from coding_agent.config import get_openai_verbosity
...
@register_command(
    name="set",
    description="Set config (e.g., /set yolo_mode true)",
    usage="/set <key> <value>",
    category="config",
)
def handle_set_command(command: str) -> bool:
    ...
    from coding_agent.messaging import emit_error, emit_info, emit_success, emit_warning
    ...
    if key:
        ...
        if key == "enable_dbos":
            emit_info(
                Text.from_markup(
                    "[bright_green]DBOS configuration changed. Please restart Coding Agent for this change to take effect.[/bright_green]"
                )
            )

        # Validate cancel_agent_key before setting
        if key == "cancel_agent_key":
            from coding_agent.keymap import VALID_CANCEL_KEYS
            ...
            emit_info(
                Text.from_markup(
                    "[bright_green]cancel_agent_key changed. Please restart Coding Agent for this change to take effect.[/bright_green]"
                )
            )

        set_config_value(key, value)
        emit_success(f'Set {key} = "{value}" in coco.cfg!')
...
def _get_json_agents_pinned_to_model(model_name: str) -> list:
    ...
    from coding_agent.agents.json_agent import discover_json_agents
...
def handle_pin_model_command(command: str) -> bool:
    ...
    if len(tokens) != 3:
        ...
        # Get built-in agents
        from coding_agent.agents.agent_manager import get_agent_descriptions
    ...
    try:
        if is_json_agent:
            ...
            from coding_agent.config import set_agent_pinned_model
...
def handle_unpin_command(command: str) -> bool:
    ...
    from coding_agent.agents.json_agent import discover_json_agents
    from coding_agent.config import get_agent_pinned_model
    from coding_agent.messaging import emit_error, emit_info, emit_success, emit_warning
    ...
    try:
        if is_json_agent:
            ...
            from coding_agent.config import clear_agent_pinned_model
...
def handle_diff_command(command: str) -> bool:
    ...
    from coding_agent.command_line.diff_menu import interactive_diff_picker
    from coding_agent.config import (
...
def handle_colors_command(command: str) -> bool:
    ...
    from coding_agent.command_line.colors_menu import interactive_colors_picker
    from coding_agent.config import set_banner_color
