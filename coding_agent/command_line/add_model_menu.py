...
from coding_agent.command_line.utils import safe_input
from coding_agent.config import EXTRA_MODELS_FILE, set_config_value
from coding_agent.list_filtering import query_matches_text
from coding_agent.messaging import emit_error, emit_info, emit_warning
from coding_agent.models_dev_parser import ModelInfo, ModelsDevRegistry, ProviderInfo
...
class AddModelMenu:
    ...
    def _add_model_to_extra_config(
        ...
        try:
            ...
            # Convert to Coding Agent config format (dictionary value)
    ...
    def _build_model_config(self, model: ModelInfo, provider: ProviderInfo) -> dict:
        """Build a Coding Agent compatible model configuration.
        ...
        """
        # Map provider IDs to Coding Agent types
        type_mapping = {
            "openai": "openai",
            "anthropic": "anthropic",
            "google": "gemini",
    ...
    def run(self) -> bool:
        ...
        try:
            ...
            # This is needed because coding_agent runs in an async context
